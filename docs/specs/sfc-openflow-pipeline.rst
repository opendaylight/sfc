...
The egress to the next hop SFF through a tunnel port becomes fairly simple:

    cookie=0xba5eba1100000101, table=10,
    priority=650,regX[8..31]={nsp},regX[0..7]={nsi}
    actions=output:1

When egress is to a different SFF through a tunnel port, if the path transport
is vxlan-gpe, the outer ethernet header will be removed:

    cookie=0xba5eba1100000101, table=10,
    priority=650,regX[8..31]={nsp},regX[0..7]={nsi}
    actions=decap(),output:1

This a sample of a SF egress flow through genius egress dispatcher:

    cookie=0xba5eba1100000202, table=87,
    priority=650,regX[8..31]={nsp},regX[0..7]={nsi}
    actions=load:0x1800->NXM_NX_REG6[],resubmit(,220)

For last hop egress, there are different scenarios to consider. When egress is
through a tunnel port with nsh encapsulation, again it will depend on the path
transport whether nsh is encapsulated with an ethernet header or not.

For example, with genius integration and vxlan-gpe transport:

    cookie=0xba5eba1100000206, table=87,
    priority=670,priority=650,regX[8..31]={nsp},regX[0..7]={nsi},nshc2=0
    actions=move:NXM_NX_NSH_C1[]->NXM_NX_TUN_IPV4_DST[],
        move:NXM_NX_NSH_C2[]->NXM_NX_TUN_ID[0..31],decap(),output:1

Same but vxlan transport:

    cookie=0xba5eba1100000204, table=87,
    priority=670,regX[8..31]={nsp},regX[0..7]={nsi},nshc2=0
    actions=move:NXM_NX_NSH_C1[]->NXM_NX_TUN_IPV4_DST[],
        move:NXM_NX_NSH_C2[]->NXM_NX_TUN_ID[0..31],output:1

When last hop egress is without nsh encapsulation:
...
Same but when tunnel destination is the local node:

    cookie=0xba5eba1100000205, table=87,
    priority=660,regX[8..31]={nsp},regX[0..7]={nsi},nshc1=2887643148
    actions=move:NXM_NX_NSH_C1[]->NXM_NX_TUN_IPV4_DST[],
        move:NXM_NX_NSH_C2[]->NXM_NX_TUN_ID[0..31],decap(),decap(),
        resubmit(,36)

Last hop when integrated with genius may rely on other local services. For
example, when relying on Netvirt L2/L3 services to forward the original
...
When relying on a different local nsh service, we resubmit a nsh packet to the
ingress dispatcher:

    cookie=0xba5eba1100000207, table=87,
    priority=680,regX[8..31]={nsp},regX[0..7]={nsi},nshc1=2887643148,nshc2=0
    actions=decap(),resubmit(,17)
