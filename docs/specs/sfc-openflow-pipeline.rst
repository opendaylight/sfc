...
it determines on what Service Chain. Once the packets are processed
by the Ingress classifier, they are sent down the rest of the Netvirt
pipeline to allow the rest of the services to process the packets.
The Egress classifier tables send the packets to the SFC tables if
necessary, be it on the same SFF (compute host) or on a different SFF.
...
+++++++++++++++++++++++++++++++++++++++++
All Nevirt Classifier will suffer minor modifications at the very least to
properly match packets by packet type.

Netvirt classifier handles nsh packets for chain termination. These may
ingress the classifier pipeline from another nsh service, typically SFC, and
it is assumed that the packet type is nsh. It may also ingress from the internal
tunnel table. For this case, a slight modification of the flows there is needed
to normalize the packet type:
...
sent to the SFC pipeline directly:
...
classifier, as read/write operation on Ethernet header fields would then apply
...
registry and the NSI on the last byte of that same registry:
    ...
    actions=load:{NSP}->regX[0..23],load:{START NSI}->regX[24..31],
        resubmit(,17)
...
restoring nsp and nsi from the registry:
    ...
    priority=260,
    actions=encap(nsh),move:regX[0..23]->nsp,move:regX[24..31]->nsi,
...
added:
    ...
    actions=encap(ethernet),load:<SFF IP>->NXM_NX_TUN_IPV4_DST,resubmit(,83)
...
added:
    ...
    cookie=0xf005ba1100000005, table=223,
    priority=260,packet_type=(1,0x894f),nsp=<NSP>
    actions=load:<SFF IP>->NXM_NX_TUN_IPV4_DST,resubmit(,83)
