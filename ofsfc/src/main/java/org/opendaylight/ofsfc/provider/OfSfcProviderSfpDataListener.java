...
public class OfSfcProviderSfpDataListener implements DataChangeListener {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(OfSfcProviderSfpDataListener.class);
    private static final OfSfcProvider odlSfc = OfSfcProvider.getOpendaylightSfcObj();

    private void installDefaultNextHopEntry(long sfpId, ServicePathHop sfpSfHop) {

        OfSfcProviderSffFlowWriter.getInstance().setNodeInfo(sfpSfHop.getServiceFunctionForwarder());

        ServiceFunction sf = SfcProviderServiceFunctionAPI.readServiceFunction(sfpSfHop.getServiceFunctionName());
        ...
        for (SfDataPlaneLocator sfDataPlanelocator : curSfDataPlaneLocatorList) {
            ...
            if (curSfLt.getImplementedInterface().equals(Mac.class)) {
                ...
                srcMac = ((MacAddressLocator) sfDataPlanelocator.getLocatorType()).getMac().getValue();
    ...
    private void writeSffFlows(ServiceFunctionPaths updatedServiceFunctionPaths) {
        ...
        while (sfpIter.hasNext()) {
            ...
            while (sfpSpHop.hasNext()) {
                ...
                OfSfcProviderSffFlowWriter.getInstance().setNodeInfo(cursfHop.getServiceFunctionForwarder());

                OfSfcProviderSffFlowWriter.getInstance().writeSffNextHopDefaultFlow();

                ServiceFunctionForwarder curSff = SfcProviderServiceForwarderAPI.readServiceFunctionForwarder(
                        odlSfc.getDataProvider(), cursfHop.getServiceFunctionForwarder());
                ...
                for (SffDataPlaneLocator curSffDataPlanelocator : curSffDataPlanelocatorList) {
                    ...
                    if (curSffLt.getImplementedInterface().equals(Mac.class)) {
                        vlan = ((MacAddressLocator) curSffDataPlanelocator.getDataPlaneLocator().getLocatorType())
                                .getVlanId();
                ...
                ServiceFunction cursf = SfcProviderServiceFunctionAPI.readServiceFunction(cursfHop
                ...
                if (oldsfHop == null) {
                    ...
                    ServiceFunctionForwarder oldSff = SfcProviderServiceForwarderAPI.readServiceFunctionForwarder(
                            odlSfc.getDataProvider(), oldsfHop.getServiceFunctionForwarder());
                    List<SffDataPlaneLocator> oldSffDataPlanelocatorList = oldSff.getSffDataPlaneLocator();

                    ServiceFunction oldsf = SfcProviderServiceFunctionAPI.readServiceFunction(oldsfHop
                            .getServiceFunctionName());
                    ...
                    if (oldSff.equals(curSff)) {
                        for (SfDataPlaneLocator curSfDataPlanelocator : curSfDataPlaneLocatorList) {
                            String dstMac = ((MacAddressLocator) curSfDataPlanelocator.getLocatorType()).getMac()
                                    .toString();
                            int dstVlan = ((MacAddressLocator) curSfDataPlanelocator.getLocatorType()).getVlanId();
                            OfSfcProviderSffFlowWriter.getInstance().writeNextHopFlow(srcMac, dstMac, dstVlan,
                                    sfp.getPathId());
                        }
                    } else {
                        for (SffDataPlaneLocator curSffDataPlanelocator : curSffDataPlanelocatorList) {
                            ...
                            for (SfDataPlaneLocator curSfDataPlanelocator : curSfDataPlaneLocatorList) {
                                ...
                                OfSfcProviderSffFlowWriter.getInstance().writeNextHopFlow(srcMacSff, dstMac, dstVlan,
                                        sfp.getPathId());
                            }
                        }
                        for (SffDataPlaneLocator oldSffDataPlanelocator : oldSffDataPlanelocatorList) {
                            String dstMacSff = ((MacAddressLocator) oldSffDataPlanelocator.getDataPlaneLocator()
                                    .getLocatorType()).getMac().toString();
                            int dstVlanSff = ((MacAddressLocator) oldSffDataPlanelocator.getDataPlaneLocator()
                                    .getLocatorType()).getVlanId();
                            // Install one flow in the old Sff
                            OfSfcProviderSffFlowWriter.getInstance()
                                    .setNodeInfo(oldsfHop.getServiceFunctionForwarder());
                            OfSfcProviderSffFlowWriter.getInstance().writeNextHopFlow(srcMac, dstMacSff, dstVlanSff,
                                    sfp.getPathId());
                        }
