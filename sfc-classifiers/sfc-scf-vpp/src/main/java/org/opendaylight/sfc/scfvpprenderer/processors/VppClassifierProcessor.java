        ...
        for (SclServiceFunctionForwarder sclSff : sfflist) {
            ...
            for (RspName rsp : rspPairList.keySet()) {
                ...
                for (Pair<HexString> maskMatch : pairList) {
                     ...
                     SfcVppUtils.configureVppClassifier(mountPoint, sffName, maskMatch.getMask(), maskMatch.getMatch(), sffInfo.pathId, sffInfo.serviceIndex, hasNext);
                }
                ...
                SfcVppUtils.addNshEntry(mountPoint, reversePathId, sffInfo.serviceIndex, sffName.getValue());
                SfcVppUtils.addNshMapWithPop(mountPoint, reversePathId, reverseServiceIndex, reversePathId, reverseServiceIndex, SfcVppUtils.buildVxlanGpePortKey(sffIp), sffName.getValue());
    ...
    private boolean removeVxlanGpeClassifier(ServiceFunctionClassifier scf) {
        ...
        for (SclServiceFunctionForwarder sclSff : sfflist) {
            ...
            for (RspName rsp : rspPairList.keySet()) {
                ...
                for (Pair<HexString> maskMatch : rspPairList.get(rsp)) {
                     SfcVppUtils.removeVppClassifier(mountPoint, sffName, maskMatch.getMatch());
                }
                SfcVppUtils.disableIngressAcl(mountPoint, itfName.get(), SfcVppUtils.buildClassifyTableKey(0), sffName.getValue());

                // Remove configuration for VPP classifier node
