...
function install_sfc {
    if [[ -n $DIST_URL ]]; then
        ...
        cd $HOME/sfc;
        #fix me: remove the workaroud
        patch -p1 < /vagrant/sfc_openflow_workaround.diff
...
function start_sfc {
    ...
    echo "log4j.logger.org.opendaylight.sfc.ofrenderer.openflow = DEBUG,stdout" >> etc/org.ops4j.pax.logging.cfg;
