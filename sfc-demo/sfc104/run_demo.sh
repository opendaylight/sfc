...
karaf=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${LOCALHOST} system:name)
if [ $? -ne 0 ] ;  then
...
sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${LOCALHOST} feature:uninstall ${uninstall_features}

#Install necessary features automatically
sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${LOCALHOST} feature:install odl-restconf ${features}
retries=6
while [ $retries -gt 0 ]
do
    installed=0
    installed_features=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${LOCALHOST} feature:list -i | grep sfc | awk '{print $1;}')
...
while [ $retries -gt 0 ]
do
    result=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${LOCALHOST} display | grep " Netconf connector initialized successfully")
    if [ $? -eq 0 ] ; then
        break
    fi
