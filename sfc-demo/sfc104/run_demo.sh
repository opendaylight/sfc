...
karaf=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${ODL_CONTROLLER} system:name)
if [ $? -ne 0 ] ;  then
    echo "Please start ODL SFC firstly."
    exit -1
fi

echo "Install and wait for sfc features: ${features}"
#Uninstall unnecessary features automatically
sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${ODL_CONTROLLER} feature:uninstall ${uninstall_features}

#Install necessary features automatically
sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${ODL_CONTROLLER} feature:install odl-restconf ${features}
retries=6
while [ $retries -gt 0 ]
do
    installed=0
    installed_features=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${ODL_CONTROLLER} feature:list -i | grep sfc | awk '{print $1;}')
...
while [ $retries -gt 0 ]
do
    result=$(sshpass -p karaf ssh -o StrictHostKeyChecking=no -p 8101 -l karaf ${ODL_CONTROLLER} display | grep " Netconf connector initialized successfully")
    if [ $? -eq 0 ] ; then
        break
    fi
