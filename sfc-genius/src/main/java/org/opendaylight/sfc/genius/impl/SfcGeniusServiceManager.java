 ...
 * locators managed by Genius. When such service functions are included in
 * a rendered service path and to be able to steer downlink traffic from
 * the service function, we need to bind SFC service to the interface
 * through Genius.
 *
 * Logical interfaces are associated to a data plane node and port. Such
 * nodes receive incoming traffic from other nodes through tunnel ports
 * managed by Genius. To be able to steer SFC uplink traffic towards the
 * service functions, we need to bind SFC service to the node through Genius.
 *
 * Logical interfaces dynamically move from one node/port to another. On
 * doing so, they become temporarily unavailable.
 */
