...
public class SfcGeniusRpcClient implements BindingAwareConsumer {
     ...
     */
    public List<Action> getEgressActionsFromGeniusRPC(String targetInterfaceName, boolean interfaceIsPartOfTheTransportZone) {
    ...
    /**Given a dpnid for a ovs switch in a compute node, the method
     * returns a list of the ip addresses associated to the compute node
     *
     * @param dpnid     the data plane node identifier
     * @return     the list of IPs for the compute node (null when not a single IP can
     *             be resolved)
     */
    public List<IpAddress> getEndpointIpForDPN(DpnIdType dpnid) {
        ...
        LOG.info("getEndpointIpForDPN: starting (dpnid={}))",
                dpnid);
        try {
            OdlInterfaceRpcService service = getInterfaceManagerRpcService();
            if (service != null) {
                LOG.info("getEndpointIpForDPN: service is not null, invoking rpc");
                RpcResult<GetEndpointIpForDpnOutput> output = service
                        .getEndpointIpForDpn(input).get();
                if (output.isSuccessful()) {
                    ips = output.getResult().getLocalIps();
                    LOG.info("getEndpointIpForDPN({}) succeeded: {}",
                            input, output);
                    successful = true;
                } else {
                    LOG.error("getEndpointIpForDPN({}) failed: {}",
                            input, output);
                }
            } else {
                LOG.error(
                        "getEndpointIpForDPN({}) failed (service couldn't be retrieved)",
                        input);
            }
        } catch (Exception e) {
            LOG.error("failed to retrieve target interface name: {} ", e);
