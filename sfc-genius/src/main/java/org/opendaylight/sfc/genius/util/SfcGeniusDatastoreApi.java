    ...
    public static Optional<MacAddress> getServiceFunctionMacAddress(String ifName) {
        ...
        return Optional.ofNullable(
                SfcGeniusUtils.getDpnIdFromLowerLayerIfList(theIf.getLowerLayerIf()).toString())
                    .map(theDpnId -> Optional.ofNullable(SfcGeniusUtilsDataGetter.getBridgeFromDpnId(theDpnId)))
                    ...
                    .map(nodeId -> Optional.ofNullable(nodeId.getValue()))
                    ...
                    .orElseThrow(() -> new RuntimeException("Could not get the ovsDB information"))
                    .stream()
                        .filter(ifExternalId -> ifExternalId.getExternalIdKey() != null &&
                                ifExternalId.getExternalIdKey().compareTo(SfcGeniusConstants.MAC_KEY) == 0)
    ...
    public static String getSfLogicalInterface(ServiceFunction sf) {
        ...
        if (theDpls.size() == 1) {
            ...
            throw new IllegalArgumentException(
                    String.format("We *must* have a single locator of type LogicalInterface. Got %s", theDpls.size()));
        }
    }
}
