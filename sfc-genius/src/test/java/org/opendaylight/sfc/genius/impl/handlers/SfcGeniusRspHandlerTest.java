...
public class SfcGeniusRspHandlerTest {
    ...
    @Mock
    SfcGeniusSfpStatesReader sfcGeniusSfpStatesReader;
    ...
    @Before
    public void setup() {
        when(sfcGeniusRspHandler.getSfReader()).thenReturn(sfcGeniusSfReader);
        when(sfcGeniusRspHandler.getSfStatesReader()).thenReturn(sfcGeniusSfStatesReader);
        when(sfcGeniusRspHandler.getSfpStatesReader()).thenReturn(sfcGeniusSfpStatesReader);
    ...
    public void interfaceStateUp() throws Exception {
        ...
        List<SfpName> sfpNameList = Arrays.asList(new SfpName("SFP1"), new SfpName("SFP2"));
        ...
        when(sfcGeniusSfReader.readSfOnInterface(testInterface))
                .thenReturn(CompletableFuture.completedFuture(sfNameList));
        when(sfcGeniusSfStatesReader.readSfpNames(sfNameList))
                .thenReturn(CompletableFuture.completedFuture(sfpNameList));
        when(sfcGeniusSfpStatesReader.readRspNames(sfpNameList))
                .thenReturn(CompletableFuture.completedFuture(rspNameList));
        ...
        sfcGeniusRspHandler.interfaceStateUp(testInterface);
