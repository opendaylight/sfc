...
public class RenderedPathListener implements DataTreeChangeListener<RenderedServicePaths> {
    ...
    private final NetconfRspProcessor rspManager;
    private final ListenerRegistration netconfRspListenerRegistration;

    public RenderedPathListener(DataBroker dataBroker, NetconfRspProcessor rspManager) {
            ...
            switch (rootNode.getModificationType()) {
                ...
                case SUBTREE_MODIFIED:
                    if (rootNode.getDataAfter() != null && rootNode.getDataAfter().getRenderedServicePath() != null) {
                        rspManager.updateRsp(rootNode.getDataAfter().getRenderedServicePath());
                    }
                    break;
                case DELETE:
                    if (rootNode.getDataBefore() != null && rootNode.getDataBefore().getRenderedServicePath() != null) {
                        rootNode.getDataBefore().getRenderedServicePath().forEach(rspManager::deleteRsp);
                    }
                    break;
            }
        }
