        ...
        Futures.addCallback(exitsingSffs, new FutureCallback<Optional<ServiceFunctionForwarders>>() {
            ...
            public void onSuccess(Optional<ServiceFunctionForwarders> optionalSffs) {
                ...
                for (ServiceFunctionForwarder sff : sffList) {
                    ...
                    // If the SFF already has the OpenFlow NodeId, then nothing to do here
                    ...
                    // Make sure its the correct OVS manager node
                    if (!SfcOvsUtil.isSffOnManagerNode(managerNode, sff)) {
                        continue;
                    }

                    LOG.info("Creating and storing augmentedSFF [{}] for bridge [{}] with DPID [{}]",
                            sff.getName().getValue(), bridgeName.getValue(), dpid.getValue());
