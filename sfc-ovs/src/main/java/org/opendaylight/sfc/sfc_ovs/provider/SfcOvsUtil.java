...
public class SfcOvsUtil {
    ...
    public static String getOpenFlowNodeIdForSff(ServiceFunctionForwarder serviceFunctionForwarder) {
        Node node = lookupTopologyNode(serviceFunctionForwarder, OpendaylightSfc.getOpendaylightSfcObj().getExecutor());
        if (node == null) {
        ...
        if (ovsdbBridge == null) {
            LOG.warn("No OvsdbBridgeAugmentaiton for Service Function Forwarder {}", serviceFunctionForwarder);
            return null;
        }
        DatapathId datapathId = getOvsDataPathId(ovsdbBridge);
        if (datapathId == null) {
    ...
    private static DatapathId getOvsDataPathId(OvsdbBridgeAugmentation ovsdbBridge) {
        Object[] methodParams = {SfcOvsUtil.buildOvsdbBridgeIID(ovsdbBridge)};
        ...
        OvsdbBridgeAugmentation readBridge =
