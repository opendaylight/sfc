...
public class SfcSffToOvsMappingAPI {
    ...
    private static List<Options> getSffDataPlaneLocatorOptions(SffDataPlaneLocator sffDataPlaneLocator) {
        ...
        try {
            OptionsBuilder optionsSrcPortBuilder = new OptionsBuilder();
            optionsSrcPortBuilder.setOption(SfcOvsUtil.OVSDB_OPTION_SRC_PORT);
            optionsSrcPortBuilder.setValue(ovsOptions.getSrcPort().getValue().toString());
            options.add(optionsSrcPortBuilder.build());
        } catch (Exception e) {
            LOG.debug("Option: {} is null.", SfcOvsUtil.OVSDB_OPTION_SRC_PORT);
        }
