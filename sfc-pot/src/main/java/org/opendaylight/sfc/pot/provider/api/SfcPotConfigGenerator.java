...
public class SfcPotConfigGenerator {
    ...
    BigInteger calculateLpc(int index, int n) {
      int count;
    ...
    void generateScvConfig() {
      ...
      ArrayList<BigInteger[]> arr = scPrecompute(lpcs, preEvalPoly2);
      lpcs = arr.get(0);
    ...
    ArrayList<BigInteger[]> scPrecompute(BigInteger[] lpcs, BigInteger[] presplitRandom) {
        int i;
        for (i = 1; i <= noOfServices; i++) {
          lpcs[i] = calculateLpc(i, noOfServices);
          presplitRandom[i] = precomputeSplitShare(serviceIndices[i], noOfServices, coeffOfPoly2);
        }
    ...
    public Short[] getServiceIndices() {
      ...
      for(int i=0; i < res.length; i++)  {
          res[i] = Short.valueOf(serviceIndices[i]);
      }
      return res;
    }
    ...
    public Long getCoeff(int i) {
      Long res = Long.valueOf(coeffOfPoly1[i]);
      return res;
    }
