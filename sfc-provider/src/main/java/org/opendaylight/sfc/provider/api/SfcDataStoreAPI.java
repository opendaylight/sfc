...
public class SfcDataStoreAPI {
    ...
    public static <U extends org.opendaylight.yangtools.yang.binding.DataObject> U readTransactionAPI
        ...
        Optional<U> serviceFunctionPathDataObject;
        CheckedFuture<Optional<U>, ReadFailedException> submitFuture = readTx.read(logicalDatastoreType, readIID);
        try {
            serviceFunctionPathDataObject = submitFuture.checkedGet();
            if (serviceFunctionPathDataObject != null
                    && serviceFunctionPathDataObject.isPresent()) {
                ret = serviceFunctionPathDataObject.get();
            } else {
                LOG.error("Failed to read Service Function Path");
            }
