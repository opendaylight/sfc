...
/*            SfcProviderServicePathAPI.checkServiceFunctionPathExecutor
    ...
    protected List<RenderedServicePathHop> createRenderedServicePathHopList(
        ...
        while(rspHopBuilderIter.hasNext()) {
            ...
            if(index == 0) {
                ...
                if(renderedServicePathHopBuilderList.size() > 1) {
                    ...
                    Class<? extends SlTransportType> rspTransportType = serviceFunctionPath.getTransportType();
                    if(rspTransportType == null) {
                        rspTransportType = rspHopSffDplMap.get(curSffName).get(0).getDataPlaneLocator().getTransport();
                    }
        ...
        if(serviceFunctionPath.isSymmetric()) {
            SfcProviderRenderedPathAPI.rspNameToRspHopSffDplList.put(
                    serviceFunctionPath.getName(), rspHopSffDplMap);
        }
        ...
        while(iter.hasPrevious()) {
            ...
            if(revServiceHop == 0) {
                if(renderedServicePathHopList.size() > 1) {
                    ...
                    //      Even though the SFP.transportType is defined with a default, if its not
                    //      specified in the configuration, it can still return null
                    Class<? extends SlTransportType> rspTransportType = renderedServicePath.getTransportType();
                    if(rspTransportType == null) {
                        rspTransportType = rspHopSffDplMap.get(revRenderedServicePathHopBuilder.getServiceFunctionForwarder()).get(0).getDataPlaneLocator().getTransport();
                    }
