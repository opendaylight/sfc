 ...
 * need to use an executor service because we cannot operate on a datastore
...
public final class SfcProviderServiceFunctionGroupAlgAPI {
     ...
     * <p>
     *
     * @param serviceFunctionGroupAlgorithmName
     *            name
     * @return ServiceFunctionGroupAlgorithm object or null if not found
     */
    protected static ServiceFunctionGroupAlgorithm readServiceFunctionGroupAlgorithm(
     ...
     *
     * @param sfgAlg
     *            the ServiceFunctionGroupAlgorithm to put
    ...
    protected static boolean putServiceFunctionGroupAlgorithm(ServiceFunctionGroupAlgorithm sfgAlg) {
        ...
        InstanceIdentifier<ServiceFunctionGroupAlgorithm> sfgAlgEntryIID = InstanceIdentifier
                ...
                .child(ServiceFunctionGroupAlgorithm.class, sfgAlg.key()).build();
    ...
    protected static boolean deleteServiceFunctionGroupAlgorithm(String serviceFunctionGroupAlgorithmName) {
        ...
        if (SfcDataStoreAPI.deleteTransactionAPI(sfgAlgEntryIID, LogicalDatastoreType.CONFIGURATION)) {
            ...
            LOG.error("{}: Could not delete SFG Algorithm: {}", Thread.currentThread().getStackTrace()[1],
                    serviceFunctionGroupAlgorithmName);
        }
        printTraceStop(LOG);
        return ret;
    ...
    public static ServiceFunctionGroupAlgorithm readServiceFunctionGroupAlg(String serviceFunctionGroupAlgName) {
        printTraceStart(LOG);
        ServiceFunctionGroupAlgorithm ret = SfcProviderServiceFunctionGroupAlgAPI
                .readServiceFunctionGroupAlgorithm(serviceFunctionGroupAlgName);
