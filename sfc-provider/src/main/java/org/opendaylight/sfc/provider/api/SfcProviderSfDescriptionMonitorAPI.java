...
public class SfcProviderSfDescriptionMonitorAPI{
    ...
    private static final InstanceIdentifier<Topology> NETCONF_NODES_PATH = InstanceIdentifier
    ...
    public GetSFDescriptionOutput getSFDescriptionInfoFromNetconf(String mountpoint)  {
        printTraceStart(LOG);
        try {
            Future<RpcResult<GetSFDescriptionOutput>> result = getDescriptionService(mountpoint).getSFDescription();
    ...
    public GetSFMonitoringInfoOutput getSFMonitorInfoFromNetconf(String mountpoint)  {
        printTraceStart(LOG);
        try {
            Future<RpcResult<GetSFMonitoringInfoOutput>> result = getDescriptionService(mountpoint).getSFMonitoringInfo();
            return result.get().getResult();
        } catch (Exception e) {
            LOG.warn("failed to ...." , e);
        }
    ...
    private ServiceFunctionDescriptionMonitorReportService getDescriptionService(String mountpoint) {
        final InstanceIdentifier<?> path = NETCONF_NODES_PATH.child(Node.class,new NodeKey(new NodeId(mountpoint)));
