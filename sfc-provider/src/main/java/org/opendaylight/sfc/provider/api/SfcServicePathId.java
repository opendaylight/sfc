...
public class SfcServicePathId {
     ...
     */
    public synchronized static long check_and_allocate_pathid(long pathid) {
     ...
     * @return True if allocated, otherwise false.
     */
    public synchronized static long check_and_allocate_pathid() {
    ...
    public static boolean check_suitable_pathid(long pathid) {
        ...
        if ((servicePathId == null) || (((1L << (64 - bitEntry)) & servicePathId.getPathIdBitarray()) == 0)) {
            // If list entry does not exist ot if it exists and associated bit in array is 0 we are good.
            return true;
        } else {
            return false;
        }
