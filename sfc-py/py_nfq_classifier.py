...
class NfqPathInfoSupplier:
    ...
    # private  - returns True if path was found
    def __add_path_info(self, path_name):
        if not get_path():
            logger.warn('__add_path_info: No path data')
            return False

        for path_item in get_path()['rendered-service-paths']['rendered-service-path']:
            if path_item['name'] == path_name:
    ...
    def get_tunnel_params(self, path_id):
        ...
        if 'context-metadata' in path_item:
            ...
            ctx_values = CONTEXTHEADER(ctx_metadata['context-header1'], ctx_metadata['context-header2'], ctx_metadata['context-header3'], ctx_metadata['context-header4'])
        else:
...
class NfqClassifierManager:
    ...
    def compile_acl(self, acl):
        ...
        result = {} # add error info to this dictionary

        for acl_item in acl['access-lists']['access-list']:
            for ace in acl_item['access-list-entries']:
                self.compile_ace(ace, result)

        return result # return info about unsuccesful ace-s

    # process_ace
    def compile_ace(self, ace, result):
        ...
        if not path_id:
            ...
            result[path_name] = 'Path data not found'
            return
    ...
    def init_new_packet_forwarder(self, path_id):
        ...
        cli = "iptables -t raw -A " + sub_chain_name + " -m mark --mark "  + str(path_id) + " -j NFQUEUE --queue-num " + str(self.nfq_number)
        execute_cli(cli)
