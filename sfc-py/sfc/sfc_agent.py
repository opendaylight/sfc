...
def build_data_plane_service_path(service_path):
    ...
    sfp1_parenth_path = sfc_globals.get_sfp_parent_path()
...
def set_odl_metadata():
    ...
    if not flask.request.json:
        ...
        local_odl_metadata = metadata_json
        print(local_context_metadata)
        """
        try:
            for mtdt in metadata_json['context-metadata']:
                local_context_metadata[mtdt['name']] = mtdt
            for mtdt in metadata_json['variable-metadata']:
                local_variable_metadata[mtdt['name']] = mtdt
            print(local_context_metadata)
            print('That was context metadata')
            print(local_variable_metadata)
            print('That was variable metadata')
        except KeyError:
            logger.info("=>No configured Metadata in ODL \n")
        """
    else:
