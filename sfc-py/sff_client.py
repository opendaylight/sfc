...
class MyUdpClient:
    def __init__(self, loop, encapsulate_type,  encapsulate_header_values, base_header_values, ctx_header_values, dest_addr, dest_port):
        self.transport = None
        self.loop = loop
    ...
    def connection_made(self, transport):
        ...
        packet = build_packet(self.encapsulate_header_values, self.encapsulate_type, self.base_header_values, self.ctx_header_values)
...
class MyGreClient:
    ...
    def send_gre_nsh(self):
        ...
        ip_saddr = socket.inet_aton ( socket.gethostbyname(socket.gethostname()) )   #Spoof the source ip address if you want to
        ...
        # Building client packet to send to SFF
        gre_nsh_packet = build_packet(self.encapsulate_header_values, self.encapsulate_type, self.base_header_values, self.ctx_header_values)
...
def main(argv):
    ...
    if trace_req:
        ...
        if encapsulate == 'vxlan':
            ...
            udpclient = MyUdpClient(loop, 'VXLAN/NSH', vxlan_header_values, base_values, ctx_values, remote_sff_ip, int(remote_sff_port))

            start_client(loop, (local_ip, 5000), (remote_sff_ip, remote_sff_port), udpclient)
        
        elif encapsulate == 'gre':
            #add GRE header
            gre_header_values = GREHEADER(int('1',2),int('000000000000',2),int('000',2),47,int('0000000000000000',2),0)

            base_values = BASEHEADER(NSH_VERSION1, int('00000000', 2), NSH_TYPE1_LEN, NSH_MD_TYPE1, NSH_NEXT_PROTO_IPV4,
                                 int(sfp_id), int(sfp_index))
            ctx_values = CONTEXTHEADER(0xffffffff, 0, 0xffffffff, 0)
            greclient = MyGreClient(loop, 'GRE/NSH', gre_header_values, base_values, ctx_values, remote_sff_ip, int(remote_sff_port))
