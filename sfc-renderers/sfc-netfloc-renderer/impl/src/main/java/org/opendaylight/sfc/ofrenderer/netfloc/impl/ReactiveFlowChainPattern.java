...
public class ReactiveFlowChainPattern implements IFlowChainPattern {
	...
	public List<Map<IBridgeOperator, List<Flow>>> apply(IServiceChain sc) {
		...
		logger.info("ReactiveFlowChainPattern Apply flow to begin path: {}", beginPath);
		...
		while (path != null && !path.equals(endPath)) {
			...
			logger.info("ReactiveFlowChainPattern Apply flow to next path: {}", path);
			path = sc.getNext(path);
		}
		hop++;
		flows.add(this.createEndPathFlows(endPath, sc.getChainId(), hop));
		logger.info("ReactiveFlowChainPattern Apply flow to end path {}", endPath);
		logger.info("ReactiveFlowChainPattern Apply flows: {}", flows);
		return flows;
	}
    ...
    matchBuilder.setEthernetMatch(ethernetMatch.build());
		...
		logger.info("ReactiveFlowChainPattern createForwardFlow: {}", flowId);
	...
	private Flow createDropFlow(IBridgeOperator bridge, IPortOperator inPort, int priority) {
		...
		logger.info("ReactiveFlowChainPattern createDropFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
	...
	private Flow createControllerFlow(IBridgeOperator bridge, IPortOperator inPort, int priority) {
		...
		logger.info("ReactiveFlowChainPattern createControllerFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
	...
	private Flow createBeginBeginBridgeFlow(IBridgeOperator bridge, int chainId, int hop, IPortOperator inPort, IPortOperator outPort, String srcMac, String dstMac, int priority) {
		...
		logger.info("ReactiveFlowChainPattern createBeginBeginBridgeFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
    ...
    matchBuilder.setEthernetMatch(ethernetMatch.build());
		...
		String flowId = "ServiceChainRewrite_" + chainId + "_" + hop + "_" + bridge.getDatapathId();
		logger.info("ReactiveFlowChainPattern createBeginBridgeFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
	...
	private Flow createBeginEndBridgeFlow(IBridgeOperator bridge, int chainId, int hop, IPortOperator inPort, IPortOperator outPort, int priority) {
		...
		logger.info("ReactiveFlowChainPattern createBeginEndBridgeFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
	...
	private Flow createEndEndBridgeFlow(IBridgeOperator bridge, int chainId, int hop, IPortOperator inPort, IPortOperator outPort, int priority) {
		...
		String flowId = "ServiceChainEndRewrite_" + chainId + "_" + hop + "_" + bridge.getDatapathId();
		logger.info("ReactiveFlowChainPattern createEndEndBridgeFlow: {}", flowId);
		flowBuilder.setId(new FlowId(flowId));
		FlowKey key = new FlowKey(new FlowId(flowId));
