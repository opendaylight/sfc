...
//                SffDataPlaneLocatorName egressDplName = sffGraph.getSffEgressDpl(entry.getDstSff(), entry.getPathId());
    ...
    private void configureTransportEgressFlows(SffGraph.SffGraphEntry entry,
        ...
        //EDIEGRA: transport egress from logical SFF to SF using a Logical Interface locator
        //1: get SF Neutron interface (read from DS)
        //2: ask genius for the egress actions for that interface
        LOG.debug("ediegra:configureTransportEgress:sfDstDpl: {}", sfDstDpl);
        ServiceFunctionForwarder sffDst =
                sfcOfProviderUtils.getServiceFunctionForwarder(entry.getDstSff(), entry.getPathId());

        if (entry.getDstDpnId() != null) {
            LOG.debug("ediegra:configureTransportEgress:entry for a logical sff!");
            if (sfDstDpl.getLocatorType().getImplementedInterface() == LogicalInterface.class) {
                //TODO ediegra put in one of those maps. it is nonsense to retrieve it twice
                LogicalInterface sfNeutronInterface = ((LogicalInterface)sfDstDpl.getLocatorType());
                ...
                LOG.debug("ediegra:configureTransportEgress:calling configureGeniusBased... with params:{} {} {} {}",
                        entry.getDstSff().toString(), entry.getPathId(), entry.getServiceIndex(), instructionList );
                transportProcessor.sfcFlowProgrammer.configureGeniusBasedTransportEgressFlow(
                       entry.getDstSff().toString(), entry.getPathId(), entry.getServiceIndex(), instructionList);
            }
        } else {
            ...
            if (sfDstDpl != null) {
                ...
                LOG.debug("ediegra:configureTransportEgress:sffSfDpl: {}", sffSfDpl);
    ...
    */
    ...
    private void initializeSff(SffGraph.SffGraphEntry entry) {
        ...
        LOG.debug("ediegra:initializeSff:retrieving sff node name. Dst sff:{} path id {}, dst dpn id:[{}]",entry.getDstSff(), entry.getPathId(), entry.getDstDpnId());
