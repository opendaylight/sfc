 ...
 *
 * - Even though method signatures use both Sff & Sf data plane locators (for keeping the
 ...
 *
 * As in {@link}SfcRspProcessorNshEth, In the future when the RspManager is finished, we
        ...
        if (entry.getDstSff().equals(SffGraph.EGRESS)) {
            ...
            if (entry.isIntraLogicalSFFEntry()) {
                ...
                LOG.debug("configureSffTransportEgressFlow: srcDpn [{}] destDpn [{}] interface to use: [{}]",
                        srcDpid, dstDpid, targetInterfaceName);
