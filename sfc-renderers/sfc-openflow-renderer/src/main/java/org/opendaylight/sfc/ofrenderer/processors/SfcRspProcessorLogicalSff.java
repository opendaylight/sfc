 ...
 *
 * <p>
 * - Even though method signatures use both Sff and Sf data plane locators (for
 * keeping the interfaces defined in the base transport processor), they are not
 * used in general (Logical SFFs don't have DPLs; SFs connected to logical SFFs
 * use Logical Interfaces as DPLs). Instead, the data plane node ids for the
 * switches participating in the hop (stored in the SffGraphEntry container) are
 * extensively used. - Transport egress actions are provided by Genius
 *
 * <p>
...
public class SfcRspProcessorLogicalSff extends SfcRspTransportProcessorBase {
    ...
    public SfcRspProcessorLogicalSff(SfcGeniusRpcClient sfcGeniusRpcClient,
            OperDsUpdateHandlerInterface operDsHandler) {
        // This transport processor relies on Genius for retrieving correct
        // table indexes. In order
        // not to create a circular dependency between this class and Genius,
        // this processor
    ...
    public void configureNextHopFlow(SffGraphEntry entry, SfDataPlaneLocator srcSfDpl, SffDataPlaneLocator dstSffDpl) {
        ...
        // (the underlying tunnels already have ips set in the tunnel mesh, only
        // transport
    ...
    public void configureSfTransportEgressFlow(SffGraphEntry entry, SffDataPlaneLocator srcSffDpl,
        ...
        // When the SF is using a logical SFF, the transport egress flows are
        // provided by Genius
        Optional<List<Action>> actionList = sfcGeniusRpcClient.getEgressActionsFromGeniusRPC(sfLogicalInterface, false,
                0);
    ...
    /**
     * Configure the Transport Egress flow from an SFF to an SFF / chain egress,
     * depending on the graph entry.
     *
     * @param entry
     *            - RSP hop info used to create the flow
     * @param srcSffDpl
        ...
        if (entry.getDstSff().equals(SffGraph.EGRESS)) {
            ...
            if (entry.isIntraLogicalSFFEntry()) {
                ...
                Optional<List<Action>> actionList = sfcGeniusRpcClient
                        .getEgressActionsFromGeniusRPC(targetInterfaceName.get(), true, 0);
     ...
     * interface locator, the method returns the SF mac address (local end) or
     * the mac address for the OVS port to which the SF is connected (remote
     * end).
     *
     * @param dstSfDpl
     *            the data plane locator
     * @param returnRemoteEnd
     *            true when the MAC for the OVS side is requested, false when
     *            the MAC for the SF side is requested
     *
     * @return the optional {@link MacAddress}
     */
    private Optional<MacAddress> getMacAddress(SfDataPlaneLocator dstSfDpl, boolean returnRemoteEnd) {
        LOG.debug("getMacAddress:starting. dstSfDpl:{}, requested side is SFF? {}", dstSfDpl, returnRemoteEnd);
        String ifName = ((LogicalInterfaceLocator) dstSfDpl.getLocatorType()).getInterfaceName();
        ...
        LOG.debug("Read interface's [{}] (remoteEndMAC requested {}) MAC address [{}]", ifName, returnRemoteEnd,
    ...
    public void configureClassifierTableMatchAny(final String sffNodeName) {
        // classifier table is not used in chains rendered by the LogicalSFF
        // processor
    }
