...
public class SfcRspProcessorMacChaining extends SfcRspTransportProcessorBase {
    ...
    */
    ...
    public void setRspTransports() {
        ...
        while (sffGraphIter.hasNext()) {
            ...
            if (!entry.getDstSff().equals(entry.getSrcSff()) &&
                    !entry.getSrcSff().equals(SffGraph.INGRESS) && !entry.getDstSff().equals(SffGraph.EGRESS)) {

                // setting Egress DPL of source SFF
                SffDataPlaneLocator srcSffDplEgress = SfcModelUtil.serachSrcDplInConnectedSffs(entry.getSrcSff(), entry.getDstSff());
                if (srcSffDplEgress == null) {
                    LOG.error(" cannot find SFF dictionary in classifier {} to  {} ", entry.getSrcSff(), entry.getDstSff());
                ...
                SffDataPlaneLocator dstSffDplIngress = SfcModelUtil.serachSrcDplInConnectedSffs(entry.getDstSff(), entry.getSrcSff());
                if (dstSffDplIngress == null) {
                    LOG.error(" cannot find SFF dictionary in classifier {} to  {} ", entry.getDstSff(), entry.getSrcSff());
    ...
    public void configureSfTransportIngressFlow(SffGraph.SffGraphEntry entry, SfDataPlaneLocator sfDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configure-Sf-TransportIngressFlow nothing to do here, just go to next table");
    ...
    public void configureSffTransportIngressFlow(SffGraph.SffGraphEntry entry, SffDataPlaneLocator dstSffDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configure-Sff-TransportIngressFlow nothing to do here, just go to next table");
        String sffNodeName = sfcProviderUtils.getSffOpenFlowNodeName(entry.getDstSff(), entry.getPathId());
        this.sfcFlowProgrammer.configureMacChainingTransportIngressFlow(sffNodeName);
    }
    ...
    public void configureSfPathMapperFlow(SffGraph.SffGraphEntry entry, SfDataPlaneLocator sfDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configure-Sf-PathMapperFlow nothing to do here, just got to next table");

    }

    @Override
    public void configureSffPathMapperFlow(SffGraph.SffGraphEntry entry, DataPlaneLocator hopDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configure-Sff-PathMapperFlow nothing to do here, just got to next table");
    ...
    public void configureNextHopFlow(SffGraph.SffGraphEntry entry, SfDataPlaneLocator srcSfDpl, SfDataPlaneLocator dstSfDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureNextHopFlow {} sf {} to sf {} ", entry.getServiceIndex(),
                srcSfDpl == null ? "null" : srcSfDpl.getName().toString(), dstSfDpl == null ? "null" : dstSfDpl.getName().toString());
        // nothing to do
    }
    ...
    public void configureNextHopFlow(SffGraph.SffGraphEntry entry, SffDataPlaneLocator srcSffDpl, SffDataPlaneLocator dstSffDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureNextHopFlow {} sff {} to sff {}", entry.getServiceIndex(),
                srcSffDpl == null ? "null" : srcSffDpl.getName().toString(), dstSffDpl == null ? "null" : dstSffDpl.getName().toString());
        // nothing to do
    }
    ...
    public void configureNextHopFlow(SffGraph.SffGraphEntry entry, SffDataPlaneLocator srcSffDpl, SfDataPlaneLocator dstSfDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureNextHopFlow {} sff {} to sf {}", entry.getServiceIndex(),
                srcSffDpl == null ? "null" : srcSffDpl.getName().toString(), dstSfDpl == null ? "null" : dstSfDpl.getName().toString());
        ...
        if (entry.getPrevSf() != null) {
            ServiceFunction prevSf = SfcProviderServiceFunctionAPI.readServiceFunction(entry.getPrevSf());
            ServiceFunctionType serviceFunctionType = SfcProviderServiceTypeAPI.readServiceFunctionType(prevSf.getType());
    ...
    public void configureNextHopFlow(SffGraph.SffGraphEntry entry, SfDataPlaneLocator srcSfDpl, SffDataPlaneLocator dstSffDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureNextHopFlow {} sf {} to sff {}", entry.getServiceIndex(),
                srcSfDpl == null ? "null" : srcSfDpl.getName().toString(), dstSffDpl == null ? "null" : dstSffDpl.getName().toString());
    ...
    public void configureSfTransportEgressFlow(SffGraph.SffGraphEntry entry, SffDataPlaneLocator srcSffDpl, SfDataPlaneLocator dstSfDpl, DataPlaneLocator hopDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureSfTransportEgressFlow {} srcSffDpl {} to dstSfDpl {}", entry.getServiceIndex(),
                srcSffDpl == null ? "null" : srcSffDpl.getName().toString(), dstSfDpl == null ? "null" : dstSfDpl.getName().toString());
    ...
    public void configureSffTransportEgressFlow(SffGraph.SffGraphEntry entry, SffDataPlaneLocator srcSffDpl, SffDataPlaneLocator dstSffDpl, DataPlaneLocator hopDpl) {
        LOG.debug("SfcRspProcessorMacChaining - configureSffTransportEgressFlow {} srcSffDpl {} to dstSffDpl {}", entry.getServiceIndex(),
                srcSffDpl == null ? "null" : srcSffDpl.getName().toString(), dstSffDpl == null ? "null" : dstSffDpl.getName().toString());
