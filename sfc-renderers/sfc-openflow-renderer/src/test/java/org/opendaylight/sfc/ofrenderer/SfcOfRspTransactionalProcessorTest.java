...
public class SfcOfRspTransactionalProcessorTest {
    ...
    private boolean areSffsFreeOfInitializationFlows(Map<Long, Map<String, List<SfcOfFlowWriterImpl.FlowDetails>>> theMap) {
        ...
        return theMap.get(SfcOfFlowProgrammerImpl.SFC_FLOWS).entrySet().stream().allMatch(emptySff);
    }
    ...
    public void testSingleRSPDeletion() throws Exception {
        ...
        Predicate<Map.Entry<String, List<SfcOfFlowWriterImpl.FlowDetails>>> emptySff =
                theInputEntry -> theInputEntry.getValue().size() == 0;
        Assert.assertTrue(theMap.get(SfcOfFlowProgrammerImpl.SFC_FLOWS).entrySet().stream().allMatch(emptySff));
    }
    ...
    public void testMultiRSPDeletionSymmetrical() throws Exception {
        ...
        Predicate<Map.Entry<String, List<SfcOfFlowWriterImpl.FlowDetails>>> emptySff =
                theInputEntry -> theInputEntry.getValue().size() == 0;
        Assert.assertTrue(rspToFlowsMap4.get(SfcOfFlowProgrammerImpl.SFC_FLOWS).entrySet().stream().allMatch(emptySff));
    }
