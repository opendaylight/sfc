...
public class SfcOfLogicalSffRspProcessorTest {
    ...
    public void testEthNshFlowCreation() throws Exception {
        ...
        // Logical SFF processor never uses table 0 as classifier (it uses genius, which uses that
        // table for service binding)
        Assert.assertEquals(0, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_CLASSIFIER)).count());
        // transport ingress: one (initialization in the only switch) + one per (hops -1)
        ...
        // (note that even thought the chain includes 2 SFs, mocks return the same dpnid for both
        // logical interfaces, i.e. simulating that both SFs are hosted in the same compute node)
        ...
        // path mapper acl: initialization only
        Assert.assertEquals(1, addedFlows.stream().filter(
    ...
    private boolean matchNextHop(Flow nextHopFlow, long rspId) {
        ...
        if (!macAddress.get().getValue().equals(theMacAddressSfSide.getValue()) && !macAddress.get().getValue().equals(theMacAddressOvsSide.getValue())) {
            return false;
        }
