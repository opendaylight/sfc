...
import static org.opendaylight.sfc.ofrenderer.openflow.SfcOfFlowProgrammerImpl.TABLE_INDEX_TRANSPORT_INGRESS;
import static org.opendaylight.sfc.ofrenderer.openflow.SfcOfFlowProgrammerImpl.TABLE_INDEX_PATH_MAPPER;
import static org.opendaylight.sfc.ofrenderer.openflow.SfcOfFlowProgrammerImpl.TABLE_INDEX_PATH_MAPPER_ACL;
import static org.opendaylight.sfc.ofrenderer.openflow.SfcOfFlowProgrammerImpl.TABLE_INDEX_NEXT_HOP;
import static org.opendaylight.sfc.ofrenderer.openflow.SfcOfFlowProgrammerImpl.TABLE_INDEX_TRANSPORT_EGRESS;
...
public class LogicalSffTests {
    ...
    @Captor
    private ArgumentCaptor<GetEgressActionsForInterfaceInput> egressActionsCaptor;
    ...
    public void testEthNshFlowCreation() throws Exception {
        ...
        // Count we have the right amount of flows in each relevant table
        ...
        Assert.assertEquals(nHops, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_TRANSPORT_INGRESS)).count());
        Assert.assertEquals(1, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_PATH_MAPPER)).count());
        Assert.assertEquals(1, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_PATH_MAPPER_ACL)).count());
        Assert.assertEquals(4, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_NEXT_HOP)).count());
        Assert.assertEquals(5, addedFlows.stream().filter(
                flow -> flow.tableKey.getId().equals(TABLE_INDEX_TRANSPORT_EGRESS)).count());
    ...
    private boolean matchTransportEgress(Flow transportEgressFlow, boolean lastHop, long rspId) {
        ...
        // LAST HOP
            ...
            // check actions - resubmit table 17
            if(!resubmit.filter(table_nr -> table_nr.equals((short)17)).isPresent()) {
                return false;
            }
        }
    ...
    private boolean matchTransportIngress(Flow transportIngressFlow, long rspId) {
        ...
        return goToTableId.filter(tableId -> tableId.equals((short)4)).isPresent();
    }
    ...
    private boolean matchNextHop(Flow nextHopFlow, long rspId) {
        ...
        // assure 1 goto table instruction; goto table 10
        Optional<Short> goToTableId = getGotoTableIdFromIntructions(nextHopFlow);
        if (!goToTableId.filter(tableId -> tableId.equals((short) 10)).isPresent()) {
            return false;
        }
