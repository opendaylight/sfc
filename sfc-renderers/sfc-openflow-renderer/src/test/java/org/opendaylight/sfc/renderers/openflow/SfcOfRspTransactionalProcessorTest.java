...
/**
 * Unit testing for the complete process of adding several Rendered Service
 * Paths (RSPs), then deleting them (in different orders). After
 * SfcOfRspProcessor receives a RSP, it transactionally creates a set of
 * OpenFlow flows. On this test, only the commit part is mocked, but several
 * checks are performed in order to verify that the expected groups of flows are
 * created / deleted at each step during the creation and deletion of several
 * RSPs.
 *
...
// We need PowerMock in order to be able to test FlowWriter internal state
// (private
// methods / lists of flows scheduled to be created / deleted at RSP creation
// time)
...
public class SfcOfRspTransactionalProcessorTest {
    ...
    private final List<SftTypeName> serviceFunctionChain2;
    private RenderedServicePath nshRsp;
    private RenderedServicePath nshRsp2;
    private final SfcOfFlowWriterInterface sfcFlowWriterTestMock;
    ...
    public SfcOfRspTransactionalProcessorTest() {
        ...
        RpcConsumerRegistry rpcConsumerRegistry = Mockito.mock(RpcConsumerRegistry.class);
        Mockito.when(rpcConsumerRegistry.getRpcService(any())).thenReturn(null);
        ...
        this.sfcOfRspProcessor = PowerMockito.spy(new SfcOfRspProcessor(this.flowProgrammerTestMock,
                this.sfcUtilsTestMock, new SfcSynchronizer(), rpcConsumerRegistry, dataBroker));
    ...
    /**
     * Helper function to check if the SFFs are free of initialization flows or
     * still have them.
     *
     * @param theMap
     *            the cache of installed flows per sff, per RSP
     * @return true if all SFFs are free of initialization flows, false
     *         otherwise
     */
    ...
    public void deleteNonExistentRsp() {
        ...
        Set<FlowDetails> flowsToDelete = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");
    ...
    /*
     * Create RSP, then delete it. - Check that all SFFs are returned to the
     * "uninitialized" state - Check that all RSPs are removed from the
     * flowWriter tables
     */
    @Test
    public void testSingleRSPDeletion() throws Exception {
        ...
        // needs to be done inside the test(it is cleared in before() method
        // between tests)
        ...
        PowerMockito.verifyPrivate(this.sfcOfRspProcessor, times(2)).invoke("setSffInitialized", anyObject(),
                Matchers.eq(true));
        ...
        Set<FlowDetails> deletedFlows = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");

        // fetch the flow cache
        Map<Long, Map<String, List<FlowDetails>>> theMap = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");

        // all the initialization flows belong to a dummy RSP - check
        // SfcOfRspProcessor SFC_FLOWS constant
        // we won't delete this entry from the cache
        assertThat(theMap.entrySet().size(), is(1));

        // get all the current flows
        Set<FlowDetails> currentFlows = theMap.entrySet().stream()
                .flatMap(rspEntry -> rspEntry.getValue().entrySet().stream())
                .flatMap(sffEntry -> sffEntry.getValue().stream()).collect(Collectors.toSet());
    ...
    /*
     * Create RSP 1, store flow count. Create RSP 2, store flows. Delete RSP2,
     * check that flows are the same than after creation of RSP1 Then Delete
     * RSP1, check that all flows are gone In summary: C:RSP1 C:RSP2 D:RSP2
     * D:RSP1, (deletion order symmetrical to creation order) checking
     * correctness
     */
    @Test
    public void testMultiRSPDeletionSymmetrical() throws Exception {
        ...
        Map<Long, Map<String, List<FlowDetails>>> rspToFlowsMap1 = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");
        ...
        LOG.debug(
                "testMultiRSPDeletionSymmetrical: After creation of first "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap1.keySet().size(), allFlowsAfterRsp1.size());
        ...
        // Store the list of flows created at this point
        Map<Long, Map<String, List<FlowDetails>>> rspToFlowsMap2 = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");
        ...
        LOG.debug(
                "testMultiRSPDeletionSymmetrical: After creation of second "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap2.keySet().size(), allFlowsAfterRsp2.size());

        // ---------------------
        // Step 3. Delete RSP2. Check that the state afterwards is the expected
        // one
        // ---------------------
        this.sfcOfRspProcessor.deleteRenderedServicePath(nshRsp2);

        // Store the list of flows created at this point
        Map<Long, Map<String, List<FlowDetails>>> rspToFlowsMap3 = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");
        verify(sfcFlowWriterTestMock).deleteFlowSet();

        Set<FlowDetails> deletedFlowsRsp2 = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");

        LOG.debug(
                "testMultiRSPDeletionSymmetrical: After deletion of RSP2: "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap3.keySet().size(), allFlowsAfterRsp2.size() - deletedFlowsRsp2.size());
        ...
        // Store the list of flows created at this point
        Map<Long, Map<String, List<FlowDetails>>> rspToFlowsMap4 = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");
        verify(sfcFlowWriterTestMock, times(2)).deleteFlowSet();

        // get the list of deleted flows belonging to RSP1
        Set<FlowDetails> allDeletedFlows = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");

        LOG.debug(
                "testMultiRSPDeletionSymmetrical: After deletion of RSP2: "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap4.keySet().size(), allDeletedFlows.size());
    ...
    /*
     * Create RSP 1, store flow count + flows themselves. Create RSP 2, store
     * flows. Delete RSP1, check that flows are the same than after creation of
     * RSP2 alone Finally Delete RSP2, check that all flows are gone In summary:
     * C:RSP1 C:RSP2 D:RSP1 D:RSP2, checking correctness along the way
     */
    @Test
    public void testMultiRSPDeletionASymmetrical() throws Exception {
        ...
        // ---------------------------------------------------------------------------------------------------------
        // Step 0(Preparation): first create RSP2, store info of created flows.
        // Then delete RSP2.
        // This is done in order to have the set of flows for RSP 2 only, so
        // that we can compare
        // with the set of flows after C:RSP1-C:RSP2-D:RSP1 (end of Step 3)
        // ---------------------------------------------------------------------------------------------------------
        this.sfcOfRspProcessor.processRenderedServicePath(nshRsp2);
        Map<Long, Map<String, List<FlowDetails>>> currentRspCache = Whitebox.getInternalState(sfcFlowWriterTestMock,
                "rspNameToFlowsMap");

        Map<Long, Map<String, List<FlowDetails>>> theRspToFlowsMapWithRSP2Only = new HashMap<>(currentRspCache);
        ...
        LOG.debug(
                "testMultiRSPDeletionASymmetrical: After creation of first "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap1.keySet().size(), allFlowsAfterRsp1.size());
        ...
        LOG.debug(
                "testMultiRSPDeletionASymmetrical: After creation of second "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap2.keySet().size(), allFlowsAfterRsp2.size());

        // ---------------------
        // Step 3. Delete RSP1. Check that the state afterwards is the expected
        // one
        // ---------------------
        this.sfcOfRspProcessor.deleteRenderedServicePath(nshRsp);
        ...
        Set<FlowDetails> deletedFlowsRsp1 = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");

        LOG.debug(
                "testMultiRSPDeletionSymmetrical: After deletion of RSP2: "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap3.keySet().size(), allFlowsAfterRsp2.size() - deletedFlowsRsp1.size());
        ...
        LOG.debug(
                "testMultiRSPDeletionASymmetrical: After deletion of RSP2: "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap3.keySet().size(), deletedFlowsRsp1.size());
        ...
        Set<FlowDetails> deleteAllFlows = Whitebox.getInternalState(sfcFlowWriterTestMock, "setOfFlowsToDelete");

        LOG.debug(
                "testMultiRSPDeletionASymmetrical: After deletion of RSP2: "
                        + "RSP: {} RSPs and {} flows stored internally by the flow writer",
                rspToFlowsMap4.keySet().size(), deleteAllFlows.size());
