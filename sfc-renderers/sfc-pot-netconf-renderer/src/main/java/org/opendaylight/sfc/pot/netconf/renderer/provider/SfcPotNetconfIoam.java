...
public class SfcPotNetconfIoam {
    ...
    public static final long DEF_NUM_POT_PROFILES = 2;
    ...
    private int processRspIoamPot (RenderedServicePath rsp,
            int sfcSize, final java.lang.Class<? extends TimeResolution> refreshPeriodTimeUnits,
            Long refreshPeriodValue, BitMaskOptions ioamPotProfileBitMask, Long ioamPotNumProfiles,
            int  currActiveIndex, boolean flagRenew) {

        Long adjRefreshPeriodValue;
        int newActiveIndex = currActiveIndex;
        PolyParameters ioamPotParams;
        SffName sffName;
        int posIndex = 0;
        ...
        if (!flagRenew) {
            ...
            if (newActiveIndex == -1) {
                LOG.error("iOAM:PoT:SB: Profile could not be renewed for RSP:{}", rsp.getName());
                return -1;
            }
        }

        ioamPotParams = potApi.getIoamPotParameters(0);
        if (ioamPotParams == null) {
            LOG.warn("iOAM:PoT:SB:profile parameters cannot be got for RSP:{}", rsp.getName());
            return -1;
        }
        ...
        for (RenderedServicePathHop h : hopList) {
            ...
            PotProfiles profile =
                buildProfile(rspName + '-' + h.getServiceIndex(), null, rspName,
                             ioamPotParams, ioamPotHopParams, posIndex, -1);
        ...
        for (RenderedServicePathHop h : hopList) {
            ...
            PotProfiles profile =
                buildProfile(rspName + '-' + h.getServiceIndex(), null, rspName,
                    ioamPotParams, ioamPotHopParams, posIndex, newActiveIndex);
            if (!configSF(configList, sffNode, profile)) {
                /* Error already logged */
                return -1;
            }
        ...
        if (!flagRenew) {
            ...
            Timeout potTimeout = potTimerWheel.setTimerContext(potTimerTask,
                                                               adjRefreshPeriodValue.longValue(),
     ...
     * @see sfc-ioam-sb-pot.yang
     */
    private PotProfiles buildProfile(String name, String acl,
                                     String rspName,
                                     PolyParameters params,
                                     PolySecrets secrets,
                                     int posIndex, int activeIndex) {
    ...
    private boolean configSF(List<Config> configList, final NodeId nodeId, PotProfiles profile) {
        ...
        if (SfcPotNetconfReaderWriterAPI.put(nodeId, LogicalDatastoreType.CONFIGURATION,
               iid, profile)) {
    ...
    private NodeId getSffNodeId(SffName sffName) {
        ...
        if (sfcForwarder == null) {
            LOG.error("iOAM:PoT:SB:SFF name {} not found in data store", sffName.getValue());
            return null;
        }

        IpAddress sffMgmtIp = sfcForwarder.getIpMgmtAddress();
        if (sffMgmtIp == null) {
            LOG.error("iOAM:PoT:SB:Unable to obtain management IP for SFF {}",
                    sffName.getValue());
            return null;
        }
    ...
    public void processRspUpdate(RenderedServicePath rsp) {
        ...
        refreshPeriodValue = potAugmentation.getRefreshPeriodValue();
        ioamPotProfileBitMask = potAugmentation.getIoamPotBitMask();
        ioamPotNumProfiles = potAugmentation.getIoamPotNumProfiles();
        ...
        processRspIoamPot (rsp, sfcSize, refreshPeriodTimeUnits,
