...
public class SfcPotNetconfIoam {
    ...
    private int sendIoamPotConfig (RenderedServicePath rsp, SfcPotPolyAPI potApi,
            List <Config> configList,  int newActiveIndex) {
        int posIndex = 0;
    ...
    private int processRspIoamPot (RenderedServicePath rsp,
        ...
        int ret = sendIoamPotConfig (rsp, potApi, configList, newActiveIndex);
        if (ret == -1) {
    ...
    public void deleteRsp(RenderedServicePath rsp) {
        ...
        if (configList != null) {
            for (Config cfg : configList) {
                broker = nodeManager.getMountPointFromNodeId(cfg.nodeId);
                SfcPotNetconfReaderWriterAPI.delete(broker, LogicalDatastoreType.CONFIGURATION, cfg.iid);
            }
