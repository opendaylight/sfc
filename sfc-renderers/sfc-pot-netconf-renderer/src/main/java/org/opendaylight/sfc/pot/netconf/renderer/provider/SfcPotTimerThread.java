...
public class SfcPotTimerThread implements Runnable {
    ...
    private SfcPotTimerThread() {
        ...
        LOG.debug("iOAM:PoT:SB:Started SFC PoT config refresher thread");
    }
        ...
        while (doLoop) {
            ...
            if (processRsp) {
                ...
                if (rspData != null) {
                    ...
                    sfcSize = rspData.getRspDataSfcSize(rspName);

                    if (sfcPotNetconfIoam != null) {
                        newActiveIndex = sfcPotNetconfIoam.refreshSfcPot(rspName, sfcSize,
                                                                         currActiveIndex,
                                                                         refreshTimerValue);
                    }
                    ...
                    Timeout potTimeout = potTimerWheel.setTimerContext(
                        rspData.getRspDataTimerTask(rspName), refreshTimerValue,
