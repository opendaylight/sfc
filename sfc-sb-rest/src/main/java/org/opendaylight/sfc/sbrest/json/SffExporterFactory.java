...
class SffExporter extends AbstractExporter implements Exporter {
    ...
    public String exportJson(DataObject dataObject) {
        ...
        if (dataObject instanceof ServiceFunctionForwarder) {
            ...
            if (sff.getIpMgmtAddress() != null) {
                sffNode.put(_IP_MGMT_ADDRESS, Util.convertIpAddress(sff.getIpMgmtAddress()));
            }
    ...
    private ObjectNode getSffDataPlaneLocatorObjectNode(SffDataPlaneLocator sffDataPlaneLocator) {
        ...
        sffDataPlaneLocatorNode.put(_DATA_PLANE_LOCATOR,
                Util.getDataPlaneLocatorObjectNode(sffDataPlaneLocator.getDataPlaneLocator()));
    ...
    private ObjectNode getSffDataPlaneLocatorOvsBridgeObjectNode(SffDataPlaneLocator sffDataPlaneLocator) {
        if (sffDataPlaneLocator == null || !(sffDataPlaneLocator instanceof SffDataPlaneLocator1)) {
            return null;
        }

        SffDataPlaneLocator1 sffDataPlaneLocator1 = (SffDataPlaneLocator1) sffDataPlaneLocator;
        ObjectNode sffDataPlaneLocatorOvsBridgeNode = mapper.createObjectNode();

        OvsBridge ovsBridge = sffDataPlaneLocator1.getOvsBridge();
        if (ovsBridge != null) {
            sffDataPlaneLocatorOvsBridgeNode.put(_BRIDGE_NAME, ovsBridge.getBridgeName());

            if (ovsBridge.getUuid() != null) {
                sffDataPlaneLocatorOvsBridgeNode.put(_UUID, ovsBridge.getUuid().getValue());
            }
            ...
            if (ovsBridge.getExternalIds() != null) {
                ...
                sffDataPlaneLocatorOvsBridgeNode.putArray(_EXTERNAL_IDS).addAll(externalIdsArray);
            }
        }

        return sffDataPlaneLocatorOvsBridgeNode;
    }
    ...
    private ObjectNode getSfDictionaryObjectNode(ServiceFunctionDictionary serviceFunctionDictionary) {
        ...
        sfObjectNode.put(_SFF_SF_DATA_PLANE_LOCATOR,
                Util.getDataPlaneLocatorObjectNode(serviceFunctionDictionary.getSffSfDataPlaneLocator()));
