...
import org.opendaylight.sfc.sbrest.provider.task.SbRestSfsTask;
...
public class SbRestSfsEntryDataListener extends SbRestAbstractDataListener {
    private static final Logger LOG = LoggerFactory.getLogger(SbRestSfsEntryDataListener.class);

    public SbRestSfsEntryDataListener(OpendaylightSfc opendaylightSfc) {
        setOpendaylightSfc(opendaylightSfc);
        setDataBroker(opendaylightSfc.getDataProvider());
        setInstanceIdentifier(OpendaylightSfc.SFS_ENTRY_IID);
    ...
    public void onDataChanged(
        ...
        for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : dataCreatedObject.entrySet()) {
            if (entry.getValue() instanceof ServiceFunctionState) {
                ...
                Runnable task = new SbRestSfsTask(RestOperation.PUT, createdServiceFunctionState, opendaylightSfc.getExecutor());
        ...
        for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : dataUpdatedObject.entrySet()) {
            if ((entry.getValue() instanceof ServiceFunctionState)
                ...
                Runnable task = new SbRestSfsTask(RestOperation.PUT, updatedServiceFunctionState, opendaylightSfc.getExecutor());
                opendaylightSfc.getExecutor().submit(task);
            }
        }
        ...
        for (InstanceIdentifier instanceIdentifier : dataRemovedConfigurationIID) {
            ...
            if (dataObject instanceof ServiceFunctionState) {
                ...
                Runnable task = new SbRestSfsTask(RestOperation.DELETE, originalServiceFunctionState, opendaylightSfc.getExecutor());
                opendaylightSfc.getExecutor().submit(task);
            }
        }
