...
import org.opendaylight.sfc.scfofrenderer.openflow.SfcOpenflowUtils;
...
public class SfcScfOfScfProcessor {
    ...
    private static FlowBuilder createClassifierFlow(short tableId, String flow, Match match,
                                                    SfcNshHeader sfcNshHeader, int outPort) {
        int order = 0;
        ...
        flowb.setId(new FlowId(flow))
             ...
             .setMatch(match)
             .setInstructions(
                 SfcOpenflowUtils.createInstructionsBuilder(
                    SfcOpenflowUtils.createActionsInstructionBuilder(setTunIpDst, setNsp, setNsi, setC1, setC2, setC3, setC4, out)).build());
        return flowb;
    }
    ...
    private void initClassifierTable(String nodeName) {
        ...
        GoToTableBuilder gotoIngress = SfcOpenflowUtils.createActionGotoTable(SfcOpenflowUtils.TABLE_INDEX_INGRESS_TRANSPORT);
        ...
        FlowBuilder fb =
             SfcOpenflowUtils.createFlowBuilder(
                 SfcOpenflowUtils.TABLE_INDEX_CLASSIFIER,
                 SfcOpenflowUtils.FLOW_PRIORITY_MATCH_ANY,
                 "MatchAny",
                 match,
                 isb);
    ...
    public void createdServiceFunctionClassifier(ServiceFunctionClassifier scf) {
        LOG.debug("\ncreate ServiceFunctionClassifier name: {} ACL: {} SFF: {}",
            scf.getName(),scf.getAccessList(), scf.getSclServiceFunctionForwarder());

        Acl acl = SfcProviderAclAPI.readAccessListExecutor(scf.getAccessList());
        ...
        for (SclServiceFunctionForwarder sclsff: sfflist) {
            ...
            for (Ace ace: acesList) {
                ...
                FlowBuilder flow = createClassifierFlow(SfcOpenflowUtils.TABLE_INDEX_CLASSIFIER, ruleName, match, nsh, outPort);
    ...
    public void deletedServiceFunctionClassifier(ServiceFunctionClassifier scf) {
        LOG.debug("\ndelete ServiceFunctionClassifier name: {} ACL: {} SFF: {}",
            scf.getName(),scf.getAccessList(), scf.getSclServiceFunctionForwarder());
        ...
        for (SclServiceFunctionForwarder sclsff: sfflist) {
            for (Ace ace: acesList) {
                ...
                removeFlowFromConfig(nodeName, new TableKey(SfcOpenflowUtils.TABLE_INDEX_CLASSIFIER),
                                                            new FlowKey(new FlowId(ruleName)));
            }
        }
    }
