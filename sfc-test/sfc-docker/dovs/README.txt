    ...
    dovs expects an environment where:

    - There is a docker image named 'docker-ovs' that runs openvswitch
    - A bridge named 'dovsbr0' with a reachable dhcp server.
    - Optionally, a reachable ODL controller through dovsbr0.
    - The utility /usr/local/sbin/pipework, to wire instance network interfaces 
      to dovsbr0 and assign IPs theough dhcp. dovsbr0 bridges the management
      network through which OVS connects to the controller. docker0 bridges
      the dataplane network.
