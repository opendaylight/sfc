...
class CMD:
    ...
    DOVS_INT = DOVS_PREFIX + 'int'
    DOCKER_IMAGE = 'docker-ovs:yyang'

    NODE_RUN = 'docker run -itd --net %s --cap-add NET_ADMIN {opts} %s' % (DOVS_MGMT, DOCKER_IMAGE)
    ...
    NODE_IP_FMT = '{{{{(index .NetworkSettings.Networks \\"dovs-int\\").IPAddress}}}}'
    NODE_PREFIXLEN_FMT = '{{{{(index .NetworkSettings.Networks \\"dovs-int\\").IPPrefixLen}}}}'
    ...
    KILL = 'kill {pid}'
    MGMT_DHCP = "ip address show %s | grep dynamic" % DOVS_MGMT
    ...
    @staticmethod
    def mgmtHasDhcp():
        return CMD.__run(CMD.MGMT_DHCP).find('dynamic') >= 0
...
class Dovs(cli.Application):
    ...
    - a docker network/bridge 'dovs-int' to inteerconnect all the containers
