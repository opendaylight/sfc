...
class ODL:
    ...
    @ifNotFound(None)
    def getTz(api, name):
        #PATCH ??? return api(ODL.TZ_RES).get(name)['transport-zone'][1]
...
class CMD:
    ...
    NODE_RUN = 'docker run -itd --cap-add NET_ADMIN {opts} %s' % DOCKER_IMAGE
    DHCP_RUN = '/usr/local/sbin/pipework %s {id} dhclient-f' % DOVS_BRIDGE
    NODE_STOP = 'docker stop {id}'
    NODE_REMOVE = 'docker rm -fv {id}'
    NODE_STATUS = 'docker inspect -f {{{{.State.Running}}}} {id}'
    NODE_IP_FMT = '{{{{.NetworkSettings.IPAddress}}}}'
    NODE_PREFIXLEN_FMT = '{{{{.NetworkSettings.IPPrefixLen}}}}'
    ...
    NS_DHCP_STOP = NS_EXEC + 'dhclient eth1 -r -pf /var/run/dhclient.{id}.pid -lf /var/lib/dhclient/dhclient.{id}.leases'
...
class Dovs(cli.Application):
    ...
    dovs expects a host environment where:

    - There is a docker image named 'docker-ovs' that runs openvswitch
    - A bridge named 'dovsbr0'
    - The utility /usr/local/sbin/pipework, to wire instance network interfaces
      to dovsbr0
    ...
    Example 1: Run a OVS instance and attach a guest namespace.
      ...
      ip stack is configured appropiately.
...
class Spawn(cli.Application):
    ...
    def main(self):
        ...
        if not len(netsinfo):
            network = subnets[0]
            nethostsit = network.hosts()
            netsinfo.append((None, network, nethostsit))
        ...
        for node in range(1, nodes + 1):
            ...
            for guest in range(1, guests + 1):
                ...
                AddGuest.invoke(
                  ...
                  ip=ip + '/' + str(netsinfo[cguest % nets][1].prefixlen),
                  net=netsinfo[cguest % nets][0],
...
class SfcConfig(cli.Application):
    ...
    - Set Service Function Chains (SFC) Configuration in Opendaylight Controller (ODL)
