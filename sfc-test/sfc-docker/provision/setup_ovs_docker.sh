...
[ ! -e build/docker_ovs_yyang.tar.gz ] && \
  echo "docker-ovs image already installed" &&
  docker load -i build/docker_ovs_yyang.tar.gz &&
  exit 0
...
ln -s "${PWD}/build/ovs_packages/openvswitch-2.5.90.tar.gz" "${TMP}/"
...
docker save docker-ovs:yyang | gzip -c > docker_ovs_yyang.tar.gz
