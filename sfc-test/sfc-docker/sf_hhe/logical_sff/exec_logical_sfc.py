...
def execSfHhe(cmd_sf_hhe, name):
    cmd='sudo ip netns exec dovs-' + name + ' python3 ' + cmd_sf_hhe + ' -i eth0 --name ' + name + ' --port 8000'
...
def execHttpServer(cmd_http_server, name):
    cmd='sudo ip netns exec dovs-' + name + ' python ' + cmd_http_server
...
def execTcpdump(guest):
    cmd='sudo ip netns exec dovs-' + guest + ' tcpdump -i eth0 -U -w ' + dir_logs + guest + '_tcpdump.pcap'
    killProcess(cmd)
...
def getCmdHttpClient(cmd_http_client, client, server, chain):
    ...
    cmd='sudo ip netns exec dovs-' + client + ' python3 ' + cmd_http_client + ' -ip ' + output_server_ip + ' --chain \"' + str_chain + '\"'
    return cmd

#server_ip=$(sudo ip netns exec dovs-${server_node} ip addr sh | grep -A 2 ": eth0" | tail -n 1 | grep -o -P '(?<=inet ).*(?=/)')
#sudo ip netns exec dovs-client1 ping -c 1 ${server_ip}
def getCmdPing(client, server):
    (status, output_server_ip) = commands.getstatusoutput('ip netns exec dovs-' + server + " ip addr sh | grep -A 2 ': eth0' | tail -n 1 | grep -o -P '(?<=inet ).*(?=/)'")
    cmd='sudo ip netns exec dovs-' + client + ' ping -c 1 ' + output_server_ip
    return cmd
...
def killProcess(cmd):
    ...
    (status, output) = commands.getstatusoutput(cmd)
    if (output != ""):
        execCommand(cmd + " | xargs sudo kill")
...
def execAllCmds(args_dir, chains, url_odl, execute_traffic):
    ...
    for sfc in chains:
        ...
        ping_cmds.append(getCmdPing(client, server))
...
def sfcConfig(args_chains, args_odl, args_different_subnets, args_only_config):
    ...
    cmd = 'sudo dovs sfc-config --chains "' + args_chains + '" --odl ' + args_odl
...
def execRemove(chains, args_odl):
    ...
    execCommand("sudo ip netns list | grep dovs | xargs -l1 sudo ip netns pids | xargs -l1 sudo kill")
    execCommand("sudo dovs clean --odl " + args_odl)
    execCommand("docker rm $(docker stop $(docker ps -q -a))")
    execCommand("sudo dovs info")
    seconds = 3
    print ("Waiting " + str(seconds) + " seconds ...")
    time.sleep(seconds)
...
def createRsps(chains, args_odl):
    ...
    for sfc in chains:
        chain_id += 1
        execCommand("sudo dovs sfc-config --create-rsp-from-id " + str(chain_id) + " --odl " + args_odl)
    time.sleep(1)
...
def deleteRsps(chains, args_odl):
    ...
    for sfc in chains:
        chain_id += 1
        execCommand("sudo dovs sfc-config --delete-rsp-from-id " + str(chain_id) + " --odl " + args_odl)
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Python3 script start Logical Service Function Chain execution',
