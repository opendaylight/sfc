...
function push_nsh {
    ...
    first_sf=$(echo ${rsp_info} | grep -o -P '(?<="service-function-name":).*(?=,)' | cut -d'"' -f 2 | sed '1,$s/\"//g' )
    last_sf=$(echo ${rsp_info} | awk 'BEGIN { FS = "service-function-name\":" } ; {print $(NF)}' | cut -d'"' -f 2 | sed '1,$s/\"//g' )
