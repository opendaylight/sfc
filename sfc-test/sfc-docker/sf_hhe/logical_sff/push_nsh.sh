...
function push_nsh {
    ...
    client_tun_port_cmd="docker exec "${client_node}" ovs-vsctl show | grep -E -B 2 \"gpe.*${first_sf_ip}\" | grep tun| cut -d'\"' -f 2"
    ...
    server_tun_port_cmd="docker exec "${server_node}" ovs-vsctl show|grep -E -B 2 \"gpe.*${last_sf_ip}\"|grep tun| cut -d'\"' -f 2"
