...
class ClientInfo(object):
    ...
    def __str__(self):
        return '%s => Ip [%s] rsp_id [%s] vlan [%s]' % (
            self.name, self.ip, self.rsp_id, self.vlan)
...
def get_cmd_line_args(context):
    ...
    if not args.do_rsp_list and not args.do_list:
        if len(args.client_name) < 1 or len(args.rsp_id) < 1:
            error_msg = ('ERROR: both the client name and rsp id '
                         'must be present to set them in the GW')
            print error_msg
            return False
        context.do_set = True
...
#
# Internal method to execute a system command and
# return the return_code and stdout as a str
#
def _execute_command(arg_list_str):
    ...
    with tempfile.TemporaryFile(mode='w+b') as f:
        try:
            ...
            print 'OS Error [%d] \"%s\", with command \"%s\"' % (
                e.errno, e.strerror, ' '.join(arg_list_str))
            return (-1, stdout_str)
        except:
            print 'Unexpected error with command: \"%s\"' % (
                ' '.join(arg_list_str))
            return (-1, stdout_str)
        ...
        if return_code != 0:
            error_msg = ('Non-zero return code [%d] for command: \"%s\"' % (
                return_code, ' '.join(arg_list_str)))
            print error_msg
...
def _get_sffs():
    (rc, sffs_str) = _execute_command(
        'sudo ovs-vsctl list-br | grep sff | grep -v node')
    if rc != 0:
...
def _get_rsp_and_vlan_mappings(context):
    all_sffs = _get_sffs()
    for sff in all_sffs:
        (rc, vlan_flows) = _execute_command(
            'sudo ovs-ofctl -O OpenFlow13 dump-flows %s table=1 |'
            ' egrep \"strip_vlan|pop_vlan\" | awk \'{print $6, $7}\'' % sff)

        if rc != 0:
            print 'Error _get_rsp_and_vlan_mappings RC %d' % rc

        # Expecting: priority=350,dl_vlan=101
        # actions=pop_vlan,write_metadata:0x1/0xffff,goto_table:3
        for flow in vlan_flows:
            ...
            if action_fields[1].startswith('write_metadata:'):
                rsp_id_hex = (action_fields[1]
                              .split('write_metadata:')[1]
                              .split('/')[0])
            ...
            if len(vlan) > 0 and len(rsp_id) > 0:
                ...
                if prev_vlan is not None:
                    if int(vlan) < int(prev_vlan):
                        # If the RSP ID already exists,
                        # only store the vlan if its smaller
                ...
                error_msg = ('ERROR could not get RSP and VLAN '
                             'from flow entry %s' % flow)
                print error_msg
...
def _get_clients(context):
    with open(MININET_DUMP_FILE) as f:
        for line in f:
            ...
            # <Host client1: client1-eth0:10.0.0.1 pid=3459>
            if line.startswith('<Host client'):
                ...
                context.clients.append(
                    ClientInfo(name=client_name, ip=client_ip))
...
def _get_client_mappings(context):
    (rc, client_mappings) = _execute_command(
        'sudo ovs-ofctl -O OpenFlow13 dump-flows gw1 table=0 | '
        'grep push_vlan | awk \'{print $6, $7}\'')

    if rc != 0:
        print 'Error get_client_mappings RC %d' % rc

    # Expecting something like this:
    #   priority=10,tcp,in_port=3,nw_src=10.0.0.1
    ...
    for client_str in client_mappings:
        ...
        for action_field in action_fields:
            if (action_field.startswith('set_field:') and
               action_field.endswith('->vlan_vid')):
                vlan = str((int(action_field
                                .split('set_field:')[1]
                                .split('->')[0])) & 0x0FFF)
...
def _get_gw2_servers():
    # When sfc-openflow-renderer_mininet.py starts, it stores the output of
    # the mininet "dump" command here: /tmp/mininet_dump.txt
...
def _get_gw_port(gw_name, conn_name):
    # When sfc-openflow-renderer_mininet.py starts, it stores the output of
    # the mininet "net" command here: /tmp/mininet_net.txt
    ...
    #    gw2 lo:  gw2-eth1:tor1-eth4 gw2-eth2:gw1-eth2 gw2-eth3:server1-eth0
    #             gw2-eth4:server2-eth0
...
def list_all_rsps(context):
    print 'There are [%d] RSP IDs defined' % len(
        context.rsp_id_to_vlan_mappings)
...
def set_client_rsp_mapping(context):
    ...
    if int(client_gw_port) < 0:
        error_msg = ('ERROR cant get switch port on gw1 '
                     'for client %s' % context.client_name)
        print error_msg
        return
    ...
    (rc, result_str) = _execute_command(
        'sudo ovs-ofctl -O OpenFlow13  add-flow gw1'
        ' "table=0,priority=10,in_port=%s,dl_type=0x0800,'
        'nw_proto=6,nw_src=%s,'
        'actions=push_vlan=0x8100,mod_vlan_vid=%s,mod_vlan_pcp=0,'
        ' output=1"' % (client_gw_port, client_info.ip, vlan))
    if rc != 0:
    ...
    for server_info in server_list:
        # TODO assuming all RSPs are symmetric,
        # and the next VLAN is offset by 100
        ...
        (rc, result_str) = _execute_command(
            ...
            ' "table=0,priority=10,in_port=%s,dl_type=0x0800,'
            'nw_proto=6,nw_dst=%s,'
            'actions=push_vlan=0x8100,mod_vlan_vid=%d,mod_vlan_pcp=0,'
            'set_field=00:00:00:00:01:01->eth_dst,output=1"' % (
                server_gw2_port, client_info.ip, return_vlan))
        if rc != 0:
