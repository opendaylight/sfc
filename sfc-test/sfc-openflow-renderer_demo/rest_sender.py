...
class Context(object):
    def __init__(self):
        ...
        self.rest_url_sf_sel = (
            'config/service-function-scheduler-type:'
...
class SffContext(object):
    def __init__(self, sff_name=None, mac=None,
                 vlan=None, port=None, sff_sf_context_list=[]):
        self.sff_name = sff_name
        self.mac = mac
        self.vlan = vlan
        self.port = port
...
def batch(context):
    ...
    if context.batch_sf_sel:
        send_rest(context, PUT,
                  context.rest_url_sf_sel,
                  context.rest_path_sf_sel)
    if context.batch_sf:
    ...
    if context.batch_rsp:
        send_rest(context, POST,
                  context.rest_url_rsp_rpc,
                  context.rest_path_rsp)
...
def CLI(context):
    ...
    while option != '0':
        ...
        print ' 9) Query Nodes'
        print '10) Change config file path, currently [%s]' % (
        ...
        if option == '1':
            send_rest(context, PUT,
                      context.rest_url_sf,
                      context.rest_path_sf)
        elif option == '2':
            send_rest(context, PUT,
                      context.rest_url_sfc,
                      context.rest_path_sfc)
        elif option == '3':
            send_rest(context, PUT,
                      context.rest_url_sff,
                      context.rest_path_sff)
        elif option == '4':
            send_rest(context, PUT,
                      context.rest_url_sfp,
                      context.rest_path_sfp)
        elif option == '5':
            send_rest(context, POST,
                      context.rest_url_rsp_rpc,
            ...
            send_rest(context, POST,
                      ...
                      context.rest_path_rsp)
            # For now, ACL is no longer used by sfc-openflow-renderer
            # Need to wait until the SFC creates the
            # RSP internally before sending the ACL
