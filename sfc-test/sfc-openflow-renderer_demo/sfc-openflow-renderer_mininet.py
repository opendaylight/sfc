...
# 2 Install openswitch version 2.2+. Visit
# https://github.com/mininet/mininet/wiki/Installing-new-version-of-Open-vSwitch

from mininet.cli import CLI
from mininet.node import Host, RemoteController, OVSController
...
if cache['vlan'].is_installed:
    ...
    error_msg = ("ERROR:VLAN package not  installed. "
                 "Please run sudo apt-get install vlan")
    print error_msg
    exit(1)
...
with open('loopback_file_pattern.sh') as f:
    LOOPBACK_FILE_PATTERN = ''.join([l.strip() for l in f.readlines()])
...
class ServiceFunctionInfo(object):
    def __init__(self,
                 name='SF', mac='00:00:00:00:00:01',
                 ip='10.0.0.1', vlan_id=0):
        self.name = name
        self.mac = mac
        self.ip = ip
...
def get_cmd_line_args(context):
    ...
    opts.add_argument(
        ...
        dest='operation_mode',
        help=('Set a VLAN or or not between the SFFs '
              'and the SFs: vlan or no-vlan'))
    ...
    context.remote_controller_args = 'remote,ip=%s,port=%s' % (
        context.remote_controller_ip, context.remote_controller_port)
    ...
    if (context.demo_mode != 'vlan') and (context.demo_mode != 'mpls'):
        error_msg = (
            "Error: Demo mode is incorrect. "
            "Try with --demo-mode vlan or --demo-mode mpls")
        print error_msg
        return False

    # Operation mode
    if ((context.operation_mode != 'vlan') and
       (context.operation_mode != 'no-vlan')):
        error_msg = ("Error: Operation mode is incorrect. "
                     "Try with --operation-mode vlan or "
                     "--operation-mode no-vlan")
        print error_msg
        return False

    # Number of switches and Service functions
    if ((int(context.switch_number) > MAX_ELEMENTS) or
       (int(context.sf_number) > MAX_ELEMENTS)):
        error_msg = ("Error: max number of openFlow switches/"
                     " Service Functions is %d" % MAX_ELEMENTS)
        print error_msg
        return False
    ...
    if int(context.sf_number) % int(context.switch_number) != 0:
        error_msg = ("Error: The number of SFs must be "
                     "a multiple of the number of SFFs")
        print error_msg
        return False
    ...
    for i in range(int(context.clients_number)):
        ...
        context.clients.append(ServiceFunctionInfo(
    ...
    for i in range(int(context.servers_number)):
        ...
        context.servers.append(ServiceFunctionInfo(
            server_name, server_mac, server_ip))
    ...
    for i in range(int(context.sf_number)):
        ...
        if context.sf_loop:
            ...
            if context.operation_mode == 'vlan':
                context.service_functions.append(
                    ServiceFunctionInfo(sf_name, sf_mac, sf_ip, sf_vlan))
            else:
                context.service_functions.append(
                    ServiceFunctionInfo(sf_name, sf_mac, sf_ip))

    # SFFs
    # NOTE: This order is important. The Openflows switches
    # have to be synchronized with the ODL configuration.
    for i in range(int(context.switch_number)):
        switch_name = "{}{}".format("sff", i+1)
        switch_protocol = context.switch_args
        context.sf_forwarders.append(
            SwitchInfo(switch_name, switch_protocol))
...
class VlanHost(Host):
    ...
    def config(self, vlan=100, **params):
        ...
        retval = self.cmd(
            'ifconfig %s.%d inet %s' % (intf, vlan, params['ip']))
        # print 'ifconfig %s.%d inet %s => result: %s' % (
        #    intf, vlan, params['ip'], retval)
...
# CREATE TOPOLOGY  ###
# 1 SFF per SF. Multiple clients can be
# linked to a GW, the same with the servers.
...
def start_switches(context, network, odl_controller, local_controller):
    for switch in network.switches:
        if switch.opts == context.switch_args:
            print 'Starting switch (%s) with ODL controller...' % (
                switch.name)
            switch.start([odl_controller])
        else:
            print 'Starting switch (%s) with local controller...' % (
                switch.name)
...
def dump_hosts(network):
    for host in network.hosts:
        print 'Host %s, IP %s, MAC %s, IFs %s' % (
            host.name, host.IP(), host.MAC(), host.intfNames())
...
def init_flows(context):
    ...
    insert_flows_by_cmd(
        'sudo ovs-vsctl set bridge gw1'
        ' protocols=OpenFlow10,OpenFlow11,OpenFlow12,OpenFlow13')
    insert_flows_by_cmd(
        'sudo ovs-vsctl set bridge gw2'
        ' protocols=OpenFlow10,OpenFlow11,OpenFlow12,OpenFlow13')
    # Match any drop flows in both GWs
    insert_flows_by_cmd(
        'sudo ovs-ofctl -O Openflow13 add-flow gw1'
        ' "table=0,priority=1,actions=drop"')
    insert_flows_by_cmd(
        'sudo ovs-ofctl -O Openflow13 add-flow gw2'
        ' "table=0,priority=1,actions=drop"')
    ...
    for client in context.clients:
        if (context.demo_mode == 'vlan'):
            ...
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=30,in_port=1,vlan_tci=0x1000/0x1000,'
                'ip,nw_dst=%s,'
                'actions=strip_vlan,set_field:%s->eth_dst,output=%d"' % (
                    client.ip_, client.mac_, gw1_out_port))

            # ARP flows
            #   Arp Req and Rsp from the client
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=20,in_port=%d,arp,actions=output=2"' % (
                    gw1_out_port))
            #   Arp Req and Rsp to the client
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=20,in_port=2,arp,arp_tpa=%s,'
                'actions=set_field:%s->eth_dst,output=%d"' % (
                    client.ip_, client.mac_, gw1_out_port))
            ...
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=10,in_port=%d,icmp,actions=output=2"' % (
                    gw1_out_port))
            #   ICMP to the client
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=10,in_port=2,icmp,nw_dst=%s,'
                'actions=set_field:%s->eth_dst,output=%d"' % (
                    client.ip_, client.mac_, gw1_out_port))
            ...
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O Openflow13 add-flow gw1'
                ' "table=0,priority=30,in_port=1,mpls,nw_dst=%s,'
                'actions=pop_mpls=0x000b,set_field:%s->eth_dst,output=%d"' % (
                    client.ip_, client.mac_, gw1_out_port))
        gw1_out_port += 1
    ...
    for server in context.servers:
        if (context.demo_mode == 'vlan'):
            ...
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O OpenFlow13 add-flow gw2'
                ' "table=0,priority=10,in_port=1,vlan_tci=0x1000/0x1000,'
                'ip,nw_dst=%s,'
                'actions=strip_vlan,set_field:%s->eth_dst,output=%d"' % (
                    server.ip_, server.mac_, gw2_out_port))
            ...
            insert_flows_by_cmd(
                'sudo ovs-ofctl -O OpenFlow13 add-flow gw2'
                ' "table=0,priority=10,in_port=1,mpls,nw_dst=%s,'
                'actions=pop_mpls=0x000b,set_field:%s->eth_dst,output=%d"' % (
                    server.ip_, server.mac_, gw2_out_port))
    ...
    if context.topology_tor:
        print 'ToR setup...'
        insert_flows_by_cmd('sudo ovs-vsctl set bridge tor1 '
                            'protocols=OpenFlow10,OpenFlow11,'
                            'OpenFlow12,OpenFlow13')
        insert_flows_by_cmd('sudo ovs-ofctl -O Openflow13 add-flow tor1'
