...
# It will look for these sorts of flows, and populate the ToR
# with the necessary info:
#    table=1, n_packets=0, priority=350,dl_vlan=101 actions=pop_vlan,
#    write_metadata:0x1/0xffff,goto_table:3
#    table=1, n_packets=0, priority=350,dl_vlan=200 actions=pop_vlan,
#    write_metadata:0x2/0xffff,goto_table:3
#    table=1, n_packets=0, priority=350,dl_vlan=500 actions=pop_vlan,
#    write_metadata:0x5/0xffff,goto_table:3
#    table=1, n_packets=0, priority=350,dl_vlan=600 actions=pop_vlan,
#    write_metadata:0x6/0xffff,goto_table:3
#
...
def _execute_command(arg_list_str):
    ...
    with tempfile.TemporaryFile(mode='w+b') as f:
        ...
        if return_code != 0:
            print 'Non-zero return code [%d] for command: \"%s\"' % (
...
def get_sff_vlans(sff):
    (rc, sff_vlan_str) = _execute_command(
...
def get_gw_egress_vlans(gw_name):
    (rc, gw_vlan_str) = _execute_command(
        'sudo ovs-ofctl -O OpenFlow13 dump-flows %s table=0 | '
        'grep push_vlan | awk \'{print $7}\'' % gw_name)
    if rc != 0:
    ...
    for gw_vlan in gw_vlan_str:
        ...
        for field in fields:
            if field.startswith('set_field:') and field.endswith('->vlan_vid'):
                gw_vlan_list.append((
                    int(field.split('set_field:')[1].split('->')[0])) & 0x00FF)
...
def get_tor_port(sff_name):
    # When sfc-openflow-renderer_mininet.py starts, it stores
    # the output of the mininet "net" command here: /tmp/mininet_net.txt
    # Looking for the following line to know the tor1 switch ports
    #    tor1 lo:  tor1-eth1:sff1-eth2 tor1-eth2:sff2-eth2
    #    tor1-eth3:gw1-eth1 tor1-eth4:gw2-eth1
...
def create_tor_vlan_flow(vlan, tor_port):
    ...
    flow_template = Template(
        'sudo ovs-ofctl -O Openflow13 add-flow tor1 "table=0,priority=10,'
        'vlan_tci=0x1000/0x1000,dl_vlan=$VLAN,actions=output=$PORT"')
    (rc, tor_flow_str) = _execute_command(
        flow_template.substitute(VLAN=vlan, PORT=tor_port))
