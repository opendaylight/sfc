...
public class GeniusServiceBinder {
    ...
    public CompletableFuture<Void> bindService(WriteTransaction theTx, String theInterfaceName, short theServiceId,
        ...
        InstanceIdentifier<BoundServices> id = InstanceIdentifier.builder(ServiceBindings.class)
                ...
                .child(BoundServices.class, new BoundServicesKey(theServiceId)).build();

        StypeOpenflow stypeOpenflow = new StypeOpenflowBuilder().setFlowCookie(theCookie)
                ...
                .build();
        BoundServices boundServices = new BoundServicesBuilder().setServiceName(theServiceName)
                .setServicePriority(theServiceId).setServiceType(ServiceTypeFlowBased.class)
    ...
    public CompletableFuture<Void> unbindService(WriteTransaction theTx, String theInterfaceName, short theServiceId) {
        InstanceIdentifier<BoundServices> id = InstanceIdentifier.builder(ServiceBindings.class)
                .child(ServicesInfo.class, new ServicesInfoKey(theInterfaceName, ServiceModeIngress.class))
                .child(BoundServices.class, new BoundServicesKey(theServiceId)).build();
