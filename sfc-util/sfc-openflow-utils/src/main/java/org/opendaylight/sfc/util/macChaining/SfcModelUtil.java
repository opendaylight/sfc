...
public class SfcModelUtil {
    ...
    public static SffDataPlaneLocator serachSrcDplInConnectedSffs(SffName sffSrcName, SffName dstSffName) {

        if (!sffSrcName.equals(dstSffName)) {
            ConnectedSffDictionary connectedSffs = SfcProviderServiceForwarderAPI.getSffSffConnectedDictionary(sffSrcName, dstSffName);
            ...
            for (SffDataPlaneLocator srcSffDpl : dplsFromSrcSff) {
                if (srcSffDpl.getDataPlaneLocator().getTransport().equals(connectedSffs.getSffSffDataPlaneLocator().getTransport())) {
                    if (srcSffDpl.getDataPlaneLocator().getLocatorType() instanceof MacAddressLocator) {
                        ...
                        MacAddressLocator macToNextSff = (MacAddressLocator) connectedSffs.getSffSffDataPlaneLocator().getLocatorType();
