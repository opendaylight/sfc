...
public class SfcVerifyNodeManager implements BindingAwareProvider {
    ...
    public SfcVerifyNodeManager(DataBroker dataBroker, BindingAwareBroker bindingAwareBroker) {
        // Register provider
        ProviderContext providerContext = bindingAwareBroker.registerProvider(this);
        onSessionInitiated(providerContext);
        // Node listener
        nodeListener = new SfcVerifyNodeListener(dataBroker, this);
    }
            ...
            if (ioamSfcvCapable) {
                ...
                if (optionalObject.isPresent()) {
                    ...
                    if (mountPoint != null) {
                        ...
                        final String streamName = SfcVerificationListener.IOAM_SCV;
                        ...
                        final Future<RpcResult<Void>> subscription =
                            rpcNotiService.createSubscription(createSubscriptionInputBuilder.build());
                        LOG.debug("Created notifications subscription for Netconf node {}...", netconfNodeId.getValue());
                    }
                }
            }
