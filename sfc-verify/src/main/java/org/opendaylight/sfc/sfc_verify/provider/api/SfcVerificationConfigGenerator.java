...
public class SfcVerificationConfigGenerator {
  ...
  private BigInteger[] presplitRandom;
  ...
  public SfcVerificationConfigGenerator(int noOfServices) {
    ...
    presplitRandom = new BigInteger[noOfServices + 1];
  ...
  void setupCoeffPrime() {
    ...
    for (i = 0; i < noOfServices; i++) {
      ...
      Random r = new Random();
      coeff[i] = (long) (numLimit <= 1 ? absLong(r.nextLong()) : absLong(r.nextLong()) % numLimit) + 1;
      coeffPerPack[i] = (long) (numLimit <= 1 ? absLong(r.nextLong()) : absLong(r.nextLong()) % numLimit) + 1;
  ...
  BigInteger precomputeSplitShare(int x, int n, Long[] coefficients) {
    ...
    int exp = 1;
    BigInteger pow_x,term, precompute_share = new BigInteger("0");;
  ...
  BigInteger calculateLpc(int index, Short[] service_indices, int n) {
    long lpc = 0;
    int count;
  ...
  BigInteger precomputeLpc(int index, Short[] serviceIndices) {
    BigInteger lpci = calculateLpc(index, serviceIndices, noOfServices);
    return lpci;
  }

  BigInteger presplitRandom(int index) {
    BigInteger presplit = precomputeSplitShare(index, noOfServices, coeffPerPack);
    return presplit;
  }
  ...
  void testCombineSecret(Short[] index) {
    ...
    for (i = 1; i <= noOfServices; i++) {
      bigLpc = calculateLpc(i, index, noOfServices);
  ...
  void testPacketFlow(BigInteger[] lpcs, Short[] index, BigInteger[] presplits) {
    ...
    for (j = 0; j < noOfPackets; j++) {
       ...
       */
      Random r = new Random();
      random2 = ((numLimit <= 1 ? absLong(r.nextLong()) : absLong(r.nextLong()) % numLimit) + 1) % limit;
      random2 /= d;
  ...
  ArrayList<BigInteger[]> testScPrecompute(BigInteger[] lpcs, Short[] index, BigInteger[] presplitRandom) {
    int i;
    for (i = 1; i <= noOfServices; i++) {
      lpcs[i] = precomputeLpc(i, index);
      presplitRandom[i] = presplitRandom(index[i]);
      if (optQUIET) {
  ...
  public Short getServiceIndices(int i) {
    Short res = serviceIndices[i + 1];
    return res;
  }

  public Short[] getServiceIndices() {
    Short[] res = serviceIndices;
    return res;
  }

  public Long getSecretShare(int i) {
    Long res = testShareSecret[i + 1];
    return res;
  }

  public BigInteger getPublicPoly(int i) {
    return presplitRandom[i];
  }
