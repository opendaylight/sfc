...
public class SfcVnfManagerProvider implements SfcVnfManager {
    ...
    public boolean createSf(ServiceFunctionType sfType) {
        ...
        TackerRequest tackerRequest =
                new TackerRequest.TackerRequestBuilder().setAuth(this.auth)
                    .setVnf(new Vnf.VnfBuilder().setName(sfType.getType().getValue())
                        .setAttributes(
                                new Attributes.AtributesBuilder().setService_type(sfType.getType().getValue()).build())
                        .build())
                    .build();
        ClientResponse response = webResource.type(javax.ws.rs.core.MediaType.APPLICATION_JSON)
            .post(ClientResponse.class, tackerRequest.toJson());

        if (response != null) {
            switch (response.getStatus()) {
                case 201:
                    String json = response.getEntity(String.class);
                    TackerResponse tackerResponse = new TackerResponse(json);
                    LOG.debug("VNF successfully created.");
                    LOG.debug(tackerResponse.toJson());
                    return true;
                case 401:
        ...
        if (response != null) {
            switch (response.getStatus()) {
                case 200:
                    ...
                    return true;
                case 400:
                    LOG.debug("Bad Request:" + response.toString());
                    return false;
                case 404:
                    LOG.debug("Not Found:" + response.toString());
                    return false;
                default:
                    LOG.debug(response.toString());
                    break;
