...
public class LispUtil {
    ...
    public static LcafTrafficEngineering buildElp(List<IpAddress> hopList) {
        LcafTrafficEngineeringBuilder elpBuilder = new LcafTrafficEngineeringBuilder();
        LcafTrafficEngineeringAddrBuilder elpAddrBuilder = new LcafTrafficEngineeringAddrBuilder();
        elpAddrBuilder.setAfi(AddressFamilyNumberEnum.LCAF.getIanaCode()).setLcafType(
                (short) LispCanonicalAddressFormatEnum.TRAFFIC_ENGINEERING.getLispCode());
        elpAddrBuilder.setHops(new ArrayList<Hops>());
        for (IpAddress hop : hopList) {
            ...
            elpAddrBuilder.getHops().add(hopsBuilder.build());
        }

        elpBuilder.setLcafTrafficEngineeringAddr(elpAddrBuilder.build());
        return elpBuilder.build();
    }
