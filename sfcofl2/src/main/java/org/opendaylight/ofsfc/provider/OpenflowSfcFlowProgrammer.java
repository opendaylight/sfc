...
public class OpenflowSfcFlowProgrammer {
		...
		Thread t = new Thread() {
			...
			public void run() {
				try {
					...
					nextHopFlow.setFlowName("default flow : " + flowId); // should this name
					// be
					// unique??
					BigInteger cookieValue = new BigInteger("20", 10);
					nextHopFlow.setCookie(new FlowCookie(cookieValue));
	...
	private void writeFlowToConfig(FlowBuilder flow) {
		...
		OpenflowSfcRenderer.getOpendaylightSfcObj().getRpcProvider().getRpcService(SalFlowService.class).addFlow(builder.build());
	}
