...
public class SfcL2RspProcessor {
    ...
    private void configurePathMapperFlows(SffGraph.SffGraphEntry entry,
        ...
        if(entry.getDstSff().equals(SffGraph.EGRESS)) {
            // TODO is this always true? even for the SFs
            return;
        }
        ...
        // TODO this is duplicated, is it necessary??
        //transportProcessor.configureSffPathMapperFlow(entry, sffDstName, false, dstHopIngressDpl, entry.getPathId(), entry.getServiceIndex());
    ...
    private void configureNextHopFlows(SffGraph.SffGraphEntry entry,
        ...
        if (entry.getDstSff().equals(SffGraph.EGRESS)) {
            ...
            // If its Egress, nothing else to be done
