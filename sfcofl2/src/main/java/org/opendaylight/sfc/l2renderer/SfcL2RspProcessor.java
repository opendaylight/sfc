...
public class SfcL2RspProcessor {
    ...
    private void configureSffNextHopFlow(final String sffName,
        ...
        configureSffNextHopFlow(sffName, srcDpl, dstDpl, srcMac, dstMac, pathId, serviceIndex);
    }

    // Simple pass-through method
    // This is the case of SFF-SFF flows
    private void configureSffNextHopFlow(final String sffName,
    ...
    private void configureSffTransportEgressFlow(final String sffName,
        ...
        LOG.info("BRADY configureSffTransportEgressFlow, srcOfsPortStr [{}]", srcOfsPortStr);
        int srcOfsPort = Integer.valueOf(srcOfsPortStr);
        String srcMac = getDictPortInfoMac(srcSffSfDict);
        String dstMac = getDplPortInfoMac(dstDpl);
        configureSffTransportEgressFlow(
                sffName, srcDpl, dstDpl, hopDpl, srcOfsPort, srcMac, dstMac, pathId, serviceIndex, isSf);
    }
    ...
    private void configureSffTransportEgressFlow(final String sffName,
        ...
        LOG.info("BRADY configureSffTransportEgressFlow srcDpl [{}] srcOfsPortStr [{}]", srcDpl, srcOfsPortStr);
        int srcOfsPort = Integer.valueOf(srcOfsPortStr);
        String srcMac = getDplPortInfoMac(srcDpl);
        String dstMac = getDplPortInfoMac(dstDpl);
        configureSffTransportEgressFlow(
    ...
    private String getDplPortInfoPort(final DataPlaneLocator dpl) {
        OfsPort ofsPort = null;
        if(dpl instanceof SffDataPlaneLocator) {
            ofsPort = getSffPortInfoFromDpl((SffDataPlaneLocator) dpl);
        } else {
            LOG.warn("getPortInfoPort() cant be used for DPLs of type SffSfDataPlaneLocator");
            return null;
        }
    ...
    private String getDplPortInfoMac(final DataPlaneLocator dpl) {
        OfsPort ofsPort = null;
        if(dpl instanceof SffDataPlaneLocator) {
            ofsPort = getSffPortInfoFromDpl((SffDataPlaneLocator) dpl);
        } else {
            LOG.warn("getPortInfoMac() cant be used for DPLs of type SffSfDataPlaneLocator");
            return null;
        }
