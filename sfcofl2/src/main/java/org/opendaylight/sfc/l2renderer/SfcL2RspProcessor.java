...
public class SfcL2RspProcessor {
    ...
    private void configureSffNextHopFlow(final String sffName,
        ...
        if (implementedInterface.equals(Ip.class)) {
            ...
            String srcMac = getSffPortInfoMac((SffDataPlaneLocator) srcDpl);
            String dstMac = getSffPortInfoMac((SffDataPlaneLocator) dstDpl);
    ...
    private void configureSffTransportEgressFlow(final String sffName,
        ...
        String srcPortStr = getSffPortInfoPort((SffDataPlaneLocator) srcDpl);
    ...
    private boolean processSffDpls(SffGraph sffGraph, final String rspTransport) {
        ...
        while (sffGraphIter.hasNext()) {
            ...
            if(entry.getSrcSff().equals(SffGraph.EGRESS)) {
                ...
                if(!setSffHopDataPlaneLocator(srcSff, rspTransport, srcSffIngressDpl, false, entry.getPathId(), sffGraph)) {
                    LOG.error("Unable to get SFF egress DPL srcSff [{}] transport [{}] pathId [{}]",
                            entry.getSrcSff(), entry.getDstSff(), rspTransport, entry.getPathId());
                    return false;
                }
            } else {
                // The srcSff egress DPL was just set, now set its ingress DPL
                ...
                if(!setSffHopDataPlaneLocator(srcSff, rspTransport, srcSffEgressDpl, true, entry.getPathId(), sffGraph)) {
     ...
     */
    private boolean setSffHopDataPlaneLocator(final ServiceFunctionForwarder sff,
    ...
    // Returns if it was possible to set the SFF DPLs
    ...
    private OfsPort getSffPortInfoFromDpl(final SffDataPlaneLocator dpl) {
        if(dpl == null) {
            return null;
        }

        SffDataPlaneLocator1 ofsDpl = dpl.getAugmentation(SffDataPlaneLocator1.class);
        if(ofsDpl == null) {
            LOG.error("No OFS DPL available for dpl [{}]", dpl.getName());
            return null;
        }
    ...
    private String getSffPortInfoPort(final SffDataPlaneLocator dpl) {
    ...
    private String getSffPortInfoMac(final SffDataPlaneLocator dpl) {
        OfsPort ofsPort = getSffPortInfoFromDpl(dpl);
        if(ofsPort == null) {
            return null;
        }
