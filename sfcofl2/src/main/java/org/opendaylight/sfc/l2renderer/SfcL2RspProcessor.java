...
public class SfcL2RspProcessor {
    ...
    private void configureSffEgress(SffGraph.SffGraphEntry entry, SffGraph sffGraph) {
        ...
        if(sfDstDpl != null) {
            if(entry.getSrcSff().equals(SffGraph.INGRESS)) {
                ...
                configureSffNextHopFlow(entry.getDstSff(), null, sfDstDpl, entry.getPathId(), entry.getServiceIndex());
            } else {
                sffSrcEgressDpl =
                        getSffDataPlaneLocator(sffSrc, sffGraph.getSffEgressDpl(entry.getSrcSff(), entry.getPathId()));
                // Configure the SFF-SFF-SF NextHop using sfDpl
                configureSffNextHopFlow(entry.getDstSff(), sffSrcEgressDpl.getDataPlaneLocator(), sfDstDpl, entry.getPathId(), entry.getServiceIndex());
            }
            ...
            configureSffTransportEgressFlow(entry.getDstSff(), sffSfDstDpl, sfDstDpl, sfDstDpl, entry.getPathId(), entry.getServiceIndex(), true);
        }
    ...
    private void configureSffNextHopFlow(final String sffName,
        ...
        if (implementedInterface.equals(Ip.class)) {
            ...
            String srcMac = getPortInfoMac(srcDpl);
            String dstMac = getPortInfoMac(dstDpl);
    ...
    private void configureSffTransportEgressFlow(final String sffName,
        ...
        String srcPortStr = getPortInfoPort(srcDpl);
        int srcPort = Integer.valueOf(srcPortStr);
        ...
        if (implementedInterface.equals(Mac.class)) {
            ...
            if(vlanTag == null) {
                this.sfcL2FlowProgrammer.configureMacTransportEgressFlow(
                        sffNodeName, srcMac, dstMac, srcPort, pathId, isSf, this.addFlow);
            } else {
                this.sfcL2FlowProgrammer.configureVlanTransportEgressFlow(
                        sffNodeName, srcMac, dstMac, vlanTag, srcPort, pathId, isSf, this.addFlow);
            }
        } else if (implementedInterface.equals(Mpls.class)) {
            // MPLS
            ...
            this.sfcL2FlowProgrammer.configureMplsTransportEgressFlow(
                    sffNodeName, srcMac, dstMac, mplsLabel, srcPort, pathId, isSf, this.addFlow);
        } else if (implementedInterface.equals(Ip.class)) {
           //VxLAN-gpe, it is IP/UDP flow with VLAN tag
           if (srcDpl != null && srcDpl.getTransport().equals(VxlanGpe.class)) {
                ...
                this.sfcL2FlowProgrammer.configureVxlanGpeTransportEgressFlow(
                        sffNodeName, srcIp, dstIp, nsp, nsi, srcPort, pathId, isSf, this.addFlow);
           }
        }
    }
    ...
    private boolean processSffDpls(SffGraph sffGraph, final String rspTransport) {
        ...
        while (sffGraphIter.hasNext()) {
            ...
            if(entry.getDstSff().equals(SffGraph.EGRESS)) {
                ...
                if(!setSffRemainingHopDataPlaneLocator(srcSff, rspTransport, srcSffIngressDpl, false, entry.getPathId(), sffGraph)) {
                ...
                if(!setSffRemainingHopDataPlaneLocator(srcSff, rspTransport, srcSffEgressDpl, true, entry.getPathId(), sffGraph)) {
    ...
    private boolean setSffRemainingHopDataPlaneLocator(final ServiceFunctionForwarder sff,
                                                       ...
                                                       SffDataPlaneLocator adjacentSffDpl,
        ...
        for(SffDataPlaneLocator sffDpl : sffDplList) {
            if(sffDpl.getName().equals(adjacentSffDpl.getName())) {
                continue;
            }
    ...
    private OfsPort getSfPortInfoFromDpl(final SffSfDataPlaneLocator sfDpl) {
        return null;
        // TODO need to get the augmenation on the service-function-dictionary, not on the SffSf DPL
        /*
        if(sfDpl == null) {
            return null;
        }
        SffDataPlaneLocator1 ofsDpl = sfDpl.getAugmentation(SffDataPlaneLocator1.class);
        if(ofsDpl == null) {
            LOG.error("No OFS DPL available for dpl [{}]", sfDpl.getName());
            return null;
        }
    ...
    private String getPortInfoPort(final DataPlaneLocator dpl) {
        OfsPort ofsPort = null;
        if(dpl instanceof SffDataPlaneLocator) {
            ofsPort = getSffPortInfoFromDpl((SffDataPlaneLocator) dpl);
        } else if(dpl instanceof SffSfDataPlaneLocator) {
            ofsPort = getSfPortInfoFromDpl((SffSfDataPlaneLocator) dpl);
        }
    ...
    private String getPortInfoMac(final DataPlaneLocator dpl) {
        OfsPort ofsPort = null;
        if(dpl instanceof SffDataPlaneLocator) {
            ofsPort = getSffPortInfoFromDpl((SffDataPlaneLocator) dpl);
        } else if(dpl instanceof SffSfDataPlaneLocator) {
