...
public class SfcL2SfgDataListener extends SfcL2AbstractDataListener {
    ...
    private void buildGroup(ServiceFunctionGroup sfg, boolean isAdd) {
        try {
            ...
            List<SfcL2FlowProgrammerInterface.GroupBucketInfo> bucketsInfo = new ArrayList<SfcL2FlowProgrammerInterface.GroupBucketInfo>();
    ...
    private SfcL2FlowProgrammerInterface.GroupBucketInfo buildBucket(ServiceFunction sf, String outPort, int index) {
        SfcL2FlowProgrammerInterface.GroupBucketInfo gbi = new SfcL2FlowProgrammerInterface.GroupBucketInfo();
        gbi.index = index;

        SfDataPlaneLocator sfDpl = sf.getSfDataPlaneLocator().get(0);
        gbi.sfMac = getSfDplMac(sfDpl);
        gbi.sfIp = getSfDplIp(sfDpl);
        gbi.outPort = outPort;
