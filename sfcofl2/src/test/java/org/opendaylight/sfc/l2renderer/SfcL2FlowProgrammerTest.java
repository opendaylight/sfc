...
public class SfcL2FlowProgrammerTest {
    ...
    public void configureArpTransportIngressFlow() {
        ...
        assertEquals(flowBuilder.getTableId().shortValue(), SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_INGRESS);
        assertEquals(flowBuilder.getPriority().intValue(),
                SfcL2FlowProgrammerOFimpl.FLOW_PRIORITY_ARP_TRANSPORT_INGRESS);
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            .getInstruction()) {
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                NxActionRegMoveNodesNodeTableFlowApplyActionsCase regMove =
                        (NxActionRegMoveNodesNodeTableFlowApplyActionsCase) action.getApplyActions()
                            .getAction()
                            .get(0)
                            .getAction();
                ...
                NxActionRegLoadNodesNodeTableFlowApplyActionsCase regLoad =
                        (NxActionRegLoadNodesNodeTableFlowApplyActionsCase) action.getApplyActions()
                            .getAction()
                            .get(2)
                            .getAction();
                ...
                assertEquals(setField.getSetField()
                    .getEthernetMatch()
                    .getEthernetSource()
                    .getAddress()
                    .getValue()
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                assertEquals(setField.getSetField()
                    ...
                    .getValue()
                    .toString(), MAC_DST);
                LOG.info("configureNextHopFlow() Action Set DstMac: [{}]", setField.getSetField()
                    .getEthernetMatch()
                    .getEthernetDestination()
                    .getAddress()
                    .getValue()
    ...
    public void configureVlanSfTransportEgressFlow() {
        ...
        sfcL2FlowProgrammer.configureVlanSfTransportEgressFlow(SFF_NAME, MAC_SRC, MAC_DST, VLAN_ID, PORT, PATH_ID_SMALL,
                PKTIN);
        flowBuilder = sfcL2FlowWriter.getFlowBuilder();

        assertEquals(flowBuilder.getTableId().shortValue(), SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS);
        assertEquals(flowBuilder.getPriority().intValue(),
                SfcL2FlowProgrammerOFimpl.FLOW_PRIORITY_TRANSPORT_EGRESS + 10);
    ...
    public void configureVlanTransportEgressFlow() {
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                LOG.info(
                        "configureVlanTransportEgressFlow() Action SrcMac is: [{}], PushVlan Ethertype: [{}], VlanId: [{}], OuputPort: [{}]",
                        setField.getSetField()
                            .getEthernetMatch()
                            .getEthernetSource()
                            .getAddress()
    ...
    public void configureVxlanGpeLastHopTransportEgressFlow() {
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                NxActionSetNshc1NodesNodeTableFlowApplyActionsCase regSet5 =
                        (NxActionSetNshc1NodesNodeTableFlowApplyActionsCase) action.getApplyActions()
                            .getAction()
                            .get(4)
                            .getAction();
                OutputActionCase output = (OutputActionCase) action.getApplyActions().getAction().get(5).getAction();
    ...
    public void configureMplsTransportEgressFlow() {
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                assertEquals(output.getOutputAction().getOutputNodeConnector().getValue(), PORT);
                LOG.info(
                        "configureMplsTransportEgressFlow() Action SrcMac: [{}], PushMplsEthertype: [{}], LabelId: [{}], OutputPort: [{}]",
                        setField.getSetField()
                            .getEthernetMatch()
                            .getEthernetSource()
                            .getAddress()
    ...
    public void configureNshNscTransportEgressFlow() {
        PowerMockito.stub(PowerMockito.method(SfcOvsUtil.class, "getVxlanOfPort")).toReturn(new Long("1"));
        sfcL2FlowProgrammer.configureNshNscTransportEgressFlow(SFF_NAME, NSP, NSI);
        flowBuilder = sfcL2FlowWriter.getFlowBuilder();
        ...
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                assertEquals(output.getOutputAction().getOutputNodeConnector().getValue(), "output:" + PORT);
    ...
    public void appCoexistenceNsh() {
        ...
        checkGoToTable(curInstruction, (short) (SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS + TABLE_BASE),
                true);
    ...
    public void appCoexistenceMpls() {
        ...
        // to the correct table and that it goes to TABLE_EGRESS
        sfcL2FlowProgrammer.configureMplsLastHopTransportEgressFlow(SFF_NAME, MAC_SRC, MAC_DST, MPLS_LABEL, PORT,
                PATH_ID);
        flowBuilder = sfcL2FlowWriter.getFlowBuilder();
        assertEquals(flowBuilder.getTableId().shortValue(),
                SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS + TABLE_BASE);
    ...
    public void appCoexistenceMatchAny() {
        ...
        assertEquals(flowBuilder.getInstructions().getInstruction().size(), 1);
        Instruction curInstruction = flowBuilder.getInstructions().getInstruction().get(0).getInstruction();
        checkGoToTable(curInstruction, (short) (SfcL2FlowProgrammerOFimpl.TABLE_INDEX_PATH_MAPPER_ACL + TABLE_BASE),
                true);
    ...
    public void checkActionHasResubmit(Instruction curInstruction, short nextTableId) {
        ...
        for (Action action : ((ApplyActionsCase) curInstruction).getApplyActions().getAction()) {
            if (action
                .getAction() instanceof org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.common.action.rev150203.actions.grouping.Action) {
                org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.common.action.rev150203.actions.grouping.Action a =
                        (org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.common.action.rev150203.actions.grouping.Action) action
                            .getAction();
