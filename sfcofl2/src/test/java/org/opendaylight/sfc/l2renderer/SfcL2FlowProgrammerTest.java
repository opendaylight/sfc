...
//        }
    ...
    public void configureGroupNextHopFlow() {
        ...
        LOG.info("configureGroupNextHopFlow table_id: [{}]", flowBuilder.getTableId());

        assertTrue(flowBuilder.getTableId() == SfcL2FlowProgrammerOFimpl.TABLE_INDEX_NEXT_HOP);

        LOG.info("configureGroupNextHopFlow() priority is: [{}]", flowBuilder.getPriority());
        ...
        LOG.info("GROUP NEXT HOP FLOW MATCH IS: [{}]", match.toString());

        Instructions isb = flowBuilder.getInstructions();
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb.getInstruction()) {

            Instruction curInstruction = instruction.getInstruction();
            LOG.info("INSTRUCTIONS FOR GROUP NEXT-HOP FLOW ARE: [{}]", curInstruction.toString());
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                LOG.info("configureGroupNextHopFlow() GROUP NAME is: [{}]", group.getGroupAction().getGroup());
                LOG.info("configureGroupNextHopFlow() GROUP ID is: [{}]", group.getGroupAction().getGroupId());
            ...
            if (curInstruction instanceof GoToTableCase) {
                GoToTableCase goToTablecase = (GoToTableCase) curInstruction;
                assertTrue(goToTablecase.getGoToTable().getTableId() == SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS);
                LOG.info("configureGroupNextHopFlow() GoTo TableId: [{}]", goToTablecase.getGoToTable().getTableId());
            }
        }
    ...
    public void configureMacTransportEgressFlow() {
        ...
        LOG.info("configureMacTransportEgressFlow table_id: [{}]", flowBuilder.getTableId());

        assertTrue(flowBuilder.getTableId() == SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS);

        LOG.info("configureMacTransportEgressFlow() priority is: [{}]", flowBuilder.getPriority());
        ...
        LOG.info("MAC TRANSPORT EGRESS IS: [{}]", match.toString());

        Instructions isb = flowBuilder.getInstructions();
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb.getInstruction()) {

            Instruction curInstruction = instruction.getInstruction();
            LOG.info("INSTRUCTIONS FOR MAC TRANSPORT EGRESS ARE: [{}]", curInstruction.toString());
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                LOG.info("configureMacTransportEgressFlow() Src Mac Adress is: [{}]", setField.getSetField().getEthernetMatch().getEthernetSource().getAddress().getValue().toString());
                LOG.info("configureMacTransportEgressFlow() OutPut Mac port is: [{}]", output.getOutputAction().getOutputNodeConnector().getValue().toString());
            }
        }
    ...
    public void configureVlanTransportEgressFlow() {
        ...
        LOG.info("configureVlanTransportEgressFlow table_id: [{}]", flowBuilder.getTableId());

        assertTrue(flowBuilder.getTableId() == SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS);

        LOG.info("configureVlanTransportEgressFlow() priority is: [{}]", flowBuilder.getPriority());
        ...
        LOG.info("VLAN TRANSPORT EGRESS IS: [{}]", match.toString());

        Instructions isb = flowBuilder.getInstructions();
        for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : isb.getInstruction()) {

            Instruction curInstruction = instruction.getInstruction();
            LOG.info("INSTRUCTIONS FOR VLAN TRANSPORT EGRESS ARE: [{}]", curInstruction.toString());
            ...
            if (curInstruction instanceof ApplyActionsCase) {
                ...
                assertTrue(output.getOutputAction().getOutputNodeConnector().getValue().toString().equals(PORT));
                LOG.info("configureMacTransportEgressFlow() Src Mac Adress is: [{}]", setField.getSetField().getEthernetMatch().getEthernetSource().getAddress().getValue().toString());
                LOG.info("configureMacTransportEgressFlow() PUSH VLAN ETHERTYPE: [{}]", pushVlan.getPushVlanAction().getEthernetType());
                LOG.info("configureMacTransportEgressFlow() VLAN ID is: [{}]", vlanId.getSetField().getVlanMatch().getVlanId().getVlanId().getValue());
                LOG.info("configureMacTransportEgressFlow() OutPut Mac port is: [{}]", output.getOutputAction().getOutputNodeConnector().getValue().toString());
            }
        }
    ...
    public void configureVxlanGpeTransportEgressFlow() {
        ...
        LOG.info("configureVxlanGpeTransportEgressFlow table_id: [{}]", flowBuilder.getTableId());

        assertTrue(flowBuilder.getTableId() == SfcL2FlowProgrammerOFimpl.TABLE_INDEX_TRANSPORT_EGRESS);

        LOG.info("configureVxlanGpeTransportEgressFlow() priority is: [{}]", flowBuilder.getPriority());
